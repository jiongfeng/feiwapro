...
class Model{
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),array('table','order','where','on','limit','having','group','lock','master','distinct','index','attr','key'),true)) {
			...
			pagecmd('setEachNum',	$args[0]);
			$this->unoptions = false;
			pagecmd('setTotalNum',	$this->get_field('COUNT(*) AS feiwa_count'));
        	return $this;
        }elseif(in_array(strtolower($method),array('min','max','count','sum','avg'),true)){
            $field =  isset($args[0])?$args[0]:'*';
            return $this->get_field(strtoupper($method).'('.$field.') AS feiwa_'.$method);
        }elseif(strtolower($method)=='count1'){
            $field =  isset($args[0])?$args[0]:'*';
            $options['field'] = ('count('.$field.') AS feiwa_count');
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
                ...
                for($i=0;$i<$count;$i++) {
                    ...
                    if('exp'==strtolower($val[$i][0])) {
                        ...
                        $feiwa = is_array($val[$i])?$this->comparison[strtolower($val[$i][0])]:'=';
                         if(preg_match('/IN/i',$feiwa)){
                         	$whereStr .= '('.$key.' '.$feiwa.' ('.$this->parseValue($data).')) '.$rule.' ';
                         }else{
                         	$whereStr .= '('.$key.' '.$feiwa.' '.$this->parseValue($data).') '.$rule.' ';
                         }
