 ...
 * @return string
 */
function xsthumb($image_name = ''){
	    if ($image_name != '') {
	list($base_name, $ext) = explode('.', $image_name);
    list($store_id) = explode('_', $base_name);
        return UPLOAD_SITE_URL.DS.ATTACH_TTM.DS.$store_id.DS.$image_name;
    }
...
{
	return str_replace(array("1", "2", "3", "4", "5", "8", "9"), array("", "抢购", "限时折扣", "优惠套装", "赠品", "", "换购"), $goods_type);
}
/**
 * 取得结算文字输出形式
 *
...
defined("ByFeiWa") || exit("Access Invalid!");
