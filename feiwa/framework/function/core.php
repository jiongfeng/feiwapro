...
function getFlexigridArray($in_array, $fields_array, $data, $format_array)//feiwa
{
	$out_array = $in_array;
...
function ncPriceFormatb($price)//feiwa
{
 ...
 * @return void
 */
function requestNotFound($app = null, $feiwa = null) {
    showMessage('您访问的页面不存在！', MALL_SITE_URL, 'exception', 'error', 1, 3000);
    exit;
}
...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	...
	switch ($show_type){
		...
		default:
			 ...
			 */
            Tpl::setDir('');
			Tpl::output('html_title',Language::get('feiwa_html_title'));
			Tpl::output('msg',$msg);
			Tpl::output('url',$url);
 ...
 * @return string 字符串类型的返回结果
 */
function getNchash($app = '', $feiwa = ''){
    $app = $app ? $app : $_GET['app'];
    $feiwa = $feiwa ? $feiwa : $_GET['feiwa'];
    if (C('captcha_status_login')){
        return substr(md5(MALL_SITE_URL.$app.$feiwa),0,8);
    } else {
        return '';
    }
}
 ...
 * 获取文章URL
 */
function getREADSArticleUrl($article_id) {
    if(URL_MODEL) {
        // 开启伪静态
        return READS_SITE_URL.DS.'article-'.$article_id.'.html';
    } else {
        return READS_SITE_URL.DS.'index.php?app=article&feiwa=article_detail&article_id='.$article_id;
    }
}
 ...
 * 获取画报URL
 */
function getREADSPictureUrl($picture_id) {
    if(URL_MODEL) {
        // 开启伪静态
        return READS_SITE_URL.DS.'picture-'.$picture_id.'.html';
    } else {
        return READS_SITE_URL.DS.'index.php?app=picture&feiwa=picture_detail&picture_id='.$picture_id;
    }
}
 ...
 * 获取文章图片URL
 */
function getREADSArticleImageUrl($image_path, $image_name, $type='list') {
    if(empty($image_name)) {
        return UPLOAD_SITE_URL.DS.ATTACH_READS.DS.'no_cover.png';
    } else {
        ...
        $file_path = ATTACH_READS.DS.'article'.DS.$image_path.DS.str_ireplace('.', '_'.$type.'.', $image_name);
        if(file_exists(BASE_PATH.DS.$file_name)) {
            $image_name = UPLOAD_SITE_URL.DS.$file_path;
        } else {
            $image_name = UPLOAD_SITE_URL.DS.ATTACH_READS.DS.'no_cover.png';
        }
        return $image_name;
    }
}
 ...
 * 获取文章图片URL
 */
function getREADSImageName($image_name_string) {
 ...
 * 获取资讯专题图片
 */
function getREADSSpecialImageUrl($image_name='') {
    return UPLOAD_SITE_URL.DS.ATTACH_READS.DS.'special'.DS.$image_name;
}

/**
 * 获取资讯专题路径
 */
function getREADSSpecialImagePath($image_name='') {
    return BASE_UPLOAD_PATH.DS.ATTACH_READS.DS.'special'.DS.$image_name;
}

/**
 * 获取资讯首页图片
 */
function getREADSIndexImageUrl($image_name='') {
    return UPLOAD_SITE_URL.DS.ATTACH_READS.DS.'index'.DS.$image_name;
}

/**
 * 获取资讯首页图片路径
 */
function getREADSIndexImagePath($image_name='') {
    return BASE_UPLOAD_PATH.DS.ATTACH_READS.DS.'index'.DS.$image_name;
}

/**
 * 获取资讯专题Url
 */
function getREADSSpecialUrl($special_id) {
    return READS_SITE_URL.DS.'index.php?app=special&feiwa=special_detail&special_id='.$special_id;
}

/**
 * 获取商城专题Url
 */
function getMallSpecialUrl($special_id) {
    return MALL_SITE_URL.DS.'index.php?app=special&feiwa=special_detail&special_id='.$special_id;
}
 ...
 * 获取资讯专题静态文件
 */
function getREADSSpecialHtml($special_id) {
    $special_file = BASE_UPLOAD_PATH.DS.ATTACH_READS.DS.'special_html'.DS.md5('special'.intval($special_id)).'.html';
 ...
 * 获取分享秀个人秀图片地址
 */
function getShareShowPersonalImageUrl($personal_info,$type=''){
    ...
    if(!empty($personal_image_list)){
        foreach ($personal_image_list as $value) {
            ...
            if(is_file(BASE_UPLOAD_PATH.DS.ATTACH_SHARESHOW.DS.$file_path)) {
                $personal_image_array[] = UPLOAD_SITE_URL.DS.ATTACH_SHARESHOW.DS.$file_path;
            } else {
                $personal_image_array[] = getShareShowDefaultImage();
            }
        }
    } else {
        $personal_image_array[] = getShareShowDefaultImage();
...
function getShareShowDefaultImage() {
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('ByFeiWa') or exit('Access Invalid!'); return ".$data.";";
 ...
 * @return obj 对象形式的返回结果
 */
function feiwa_class($classname = null){
	static $_cache = array();
...
function snsShareFrom($sign) {
    switch ($sign) {
        case '1' :
        case '2' :
            return L('sns_from') . '<a target="_black" href="' . MALL_SITE_URL . '">' . L('sns_mall') . '</a>';
            break;
        case '3' :
            return L('sns_from') . '<a target="_black" href="' . SHARESHOW_SITE_URL . '">' . L('feiwa_modules_shareshow') . '</a>';
            break;
        case '4' :
            return L('sns_from') . '<a target="_black" href="' . READS_SITE_URL . '">' . L('feiwa_reads') . '</a>';//资讯
            break;
        case '5' :
            return L('sns_from') . '<a target="_black" href="' . CIRCLE_SITE_URL . '">' . L('feiwa_circle') . '</a>';
            break;
    }
}
 ...
 *
 * url('search,'index','array('cate_id'=>2)); 实际指向 index.php?app=search&feiwa=index&cate_id=2
 * 传递数组参数时，若app（或feiwa）值为index,则可以省略
 * 上面示例等同于
 * url('search','',array('app'=>'search','cate_id'=>2));
 *
 * @param string $app control文件名
 * @param string $feiwa op方法名
 * @param array $args URL其它参数
 * @param boolean $model 默认取当前系统配置
 * @param string $site_url 生成链接的网址，默认取当前网址
 * @return string
 */
function url($app = '', $feiwa = '', $args = array(), $model = false, $site_url = ''){
    //伪静态文件扩展名
    $ext = '.html';
    //入口文件名
    $file = 'index.php';
//    $site_url = empty($site_url) ? MALL_SITE_URL : $site_url;
    ...
    //定义变量存放返回url
    $url_string = '';
    if (empty($app) && empty($feiwa) && empty($args)) {
        return $site_url;
    }
    $app = !empty($app) ? $app : 'index';
    $feiwa = !empty($feiwa) ? $feiwa : 'index';

    $model = $model ? URL_MODEL : $model;

    if ($model) {
        //伪静态模式
        $url_perfix = "{$app}-{$feiwa}";
        if (!empty($args)){
            $url_perfix .= '-';
        }
        ...
        //默认路由模式
        $url_perfix = "app={$app}&feiwa={$feiwa}";
        if (!empty($args)){
            $url_perfix .= '&';
        }
    ...
    $reg_match_to = array(
        ...
        'c-\\1.html',
        't-\\1.html',
        'mall_view-\\1-\\2-\\3-\\4-\\5.html',
        'article-\\1.html',
        'article_list-\\1.html',
        'article_cate-\\1.html',
        'document-\\1.html',
        's-\\1-\\2-\\3-\\4-\\5-\\6-\\7-\\8-\\9.html',
		'channel-\\1.html',
 ...
 * @return string
 */
function urlMall($app = '', $feiwa = '', $args = array(), $store_domain = ''){

    // 如何使自营店则返回javascript:;

//    if ($app == 'show_store' && $feiwa != 'goods_all') {
//        static $ownMallIds = null;
//        if ($ownMallIds === null) {
//            $ownMallIds = Model('store')->getOwnMallIds();
//        }
//        if (isset($args['store_id']) && in_array($args['store_id'], $ownMallIds)) {
//            return 'javascript:;';
//        }
//    }
    ...
    if(URL_MODEL && in_array($app.':'.$feiwa, $rewrite_item)) {
        $rewrite_flag = true;
    ...
    return url($app, $feiwa, $args, $rewrite_flag, MALL_SITE_URL);
}
 ...
 * @return string
 */
function urlAdmin($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, ADMIN_SITE_URL);
}

/**
 * 商城后台mall使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $feiwa op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlAdminMall($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, ADMIN_modules_URL);
}
/**
 ...
 * @return string
 */
function urlAdminCircle($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, CIRCLE_modules_URL);
}
/**
 ...
 */
function urlAdminMobile($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, MOBILE_modules_URL);
}
/**
 * 商城后台Reads使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $feiwa op方法名
 * @param array $args URL其它参数
 * @return stringurlAdminMobile
 */
function urlAdminReads($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, READS_modules_URL);
}
/**
 * 商城后台SHARESHOW使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $feiwa op方法名
 * @param array $args URL其它参数
 * @return stringurlAdminMobile
 */
function urlAdminShareShow($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, SHARESHOW_modules_URL);
}
/**
 * 資訊使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $feiwa op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlREADS($app = '', $feiwa = '', $args = array()){
	// 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
    ...
    return url($app, $feiwa, $args, $rewrite_flag, READS_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlCircle($app = '', $feiwa = '', $args = array()){
	// 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
	        'index:index',
	        'theme:theme_detail'
    ...
    return url($app, $feiwa, $args, $rewrite_flag, CIRCLE_SITE_URL);
}
/**
 * 分享秀使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $app control文件名
 * @param string $feiwa op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlShareShow($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, SHARESHOW_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlMember($app = '', $feiwa = '', $args = array()) {
    // 默认标志为不开启伪静态
    $rewrite_flag = false;
    ...
    return url($app, $feiwa, $args, $rewrite_flag, MEMBER_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlLogin($app = '', $feiwa = '', $args = array()) {
	    // 如果平台开启伪静态开关，并且为伪静态模块，修改标志为开启伪静态
    $rewrite_item = array(
	        'login:index',
			'login:register'
    ...
    return url($app, $feiwa, $args, false, LOGIN_SITE_URL);
}
/**
 ...
 * @return string
 */
function urlChain($app = '', $feiwa = '', $args = array()){
    return url($app, $feiwa, $args, false, CHAIN_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
...
function checkPlatformStore($store_id=null){
	if($store_id == null) {
		return $_SESSION['is_own_mall'];
	}
	else
	{
		if(isset($_SESSION['is_own_mall'])){
			 return $_SESSION['is_own_mall'] ;
		}
		$store_model = Model('store');
		$store = $store_model->getStoreInfoByID($store_id) ;
		return $store['is_own_mall'] ;
	}
}
