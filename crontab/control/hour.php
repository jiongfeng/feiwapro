...
class hourControl extends BaseCronControl {
     ...
     * 全量创建索引
     */
    public function xs_createFeiwa() {
        ...
        try {
            ...
            if ($count != 0) {
                for ($i = 0; $i <= $count; $i = $i + $step_num){
                    if (C('dbdriver') == 'mysql') {
                        $goods_list = $model_goods->getGoodsOnlineList(array(), '*,'.$_field.' feiwa_distinct', 0, '', "{$i},{$step_num}", $_distinct);
                    } elseif (C('dbdriver') == 'oracle') {
    ...
    public function _xs_update() {
        ...
        try {
            ...
            for ($i = 0; $i <= $count; $i = $i + $step_num){
                if (C('dbdriver') == 'mysql') {
                    $goods_list = $model_goods->getGoodsOnlineList($condition, '*,'.$_field.' feiwa_distinct', 0, '', "{$i},{$step_num}", $_distinct);
                } elseif (C('dbdriver') == 'oracle') {
                    //先查出所有goods_id,再使用in查询
                    $condition['goods_state']   = 1;
                    $condition['goods_verify']  = 1;
    ...
    private function _build_goods($goods_list = array()) {
        ...
        foreach ($goods_list as $k => $v) {
            ...
            // 判断店铺是否为自营店铺
            $index_data['store_id'] = $v['is_own_mall'];
    ...
    public function xs_flushLoggingFeiwa(){
        if (!C('fullindexer.open')) return;
        $this->_ini_xs();
        try {
            $this->_index->flushLogging();
    ...
    public function xs_flushIndexFeiwa(){
        if (!C('fullindexer.open')) return;
        $this->_ini_xs();
