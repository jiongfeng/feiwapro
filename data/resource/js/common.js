...
function login_dialog(){
    CUR_DIALOG = ajax_form('login','登录',MEMBER_SITE_URL+'/index.php?app=login&inajax=1',360,1);
}
...
function collect_store(fav_id,jstype,jsobj){
    $.get('index.php?app=index&feiwa=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?app=member_favorite_store&feiwa=favoritestore';
            $.getJSON(url, {'fid':fav_id}, function(data){
                ...
                if(jstype == 'store'){
                    $('[feiwa_store="'+fav_id+'"]').each(function(){
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('index.php?app=index&feiwa=login', function(result){
        if(result=='0'){
            ...
            var url = 'index.php?app=member_favorite_goods&feiwa=favoritegoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
                showDialog(data.msg, 'succ','','','','','','','','',2);
...
function load_cart_information(){
	$.getJSON(SITEURL+'/index.php?app=cart&feiwa=ajax_load&callback=?', function(result){
...
function drop_topcart_item(cart_id){
    $.getJSON(SITEURL+'/index.php?app=cart&feiwa=del&cart_id='+cart_id+'&callback=?', function(result){
        if(result.state){
            var obj = $('.mod_minicart');
            //删除成功
            if(result.quantity == 0){
    	        ...
    	        if ($('#rtoolbar_cartlist > ul').children().size() != result.quantity) {
    	        	$("#rtoolbar_cartlist").load(MALL_SITE_URL+ '/index.php?app=cart&feiwa=ajax_load&type=html');return ;
    	        }
            }
        }else{
            alert(result.msg);
...
function load_history_information(){
    $.getJSON(SITEURL+'/index.php?app=index&feiwa=viewed_info', function(result){
...
(function($) {
    $.show_feiwa_login = function(options) {
        var settings = $.extend({}, {action:'/index.php?app=login&feiwa=index', nchash:'', formhash:'' ,anchor:''}, options);
        var login_dialog_html = $('<div class="quick-login"></div>');
        var ref_url = document.location.href;
        var add_html = '<span class="other">';
        if (connect_qq == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php?app=connect_qq" title="QQ账号登录" class="qq"><i></i></a>';
        }
        if (connect_sn == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php?app=connect_sina" title="新浪微博账号登录" class="sina"><i></i></a>';
        }
        if (connect_wx == '1'){
            add_html += '<a href="javascript:void(0);" onclick="ajax_form(\'weixin_form\', \'微信账号登录\', \''+LOGIN_SITE_URL+'/index.php?app=connect_wx&feiwa=index\', 360);" title="微信账号登录" class="wx"><i></i></a>';
        }
        add_html += '</span>';
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+LOGIN_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'index.php?app=seccode&feiwa=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="index.php?app=seccode&feiwa=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>')
        	.append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="'+LOGIN_SITE_URL+'/index.php?app=login&amp;wwi=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+LOGIN_SITE_URL+'/index.php?app=login&feiwa=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
    ...
    $.fn.feiwa_login = function(options) {
        return this.each(function() {
            $(this).on('click',function(){
                $.show_feiwa_login(options);
                return false;
            });
        });
    };
 ...
 */
(function($) {
    $.fn.feiwa_show_dialog = function(options) {
...
(function($){
	$.fn.membershipCard = function(options){
		var defaults = {
				type:''			// params  mall/circle/reads/micormall
			};
		options = $.extend(defaults,options);
		return this.each(function(){
			var $this = $(this);
			var data_str = $(this).attr('data-param');eval('data_str = '+data_str);
			var _uri = SITEURL+'/index.php?app=member_card&callback=?&uid='+data_str.id+'&from='+options.type;
			var _dl = '';
			$this.qtip({
	            content: {
	                text: 'Loading...',
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
	                			...
	                			.append('<a href="'+MALL_SITE_URL+'/index.php?app=member_snshome&mid='+data.id+'" target="_blank">'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''));
		                		//show membergrade
		                		...
		                		// avatar
		                		$('<dd class="avatar"><a href="'+MALL_SITE_URL+'/index.php?app=member_snshome&mid='+data.id+'" target="_blank"><img src="'+data.avatar+'" /></a><dd>')
		                			.appendTo(_dl);
		                		// info
		                		var _info = '';
		                		...
		                		// ajax info
		                		if(data.url != ''){
			                		$.getJSON(data.url+'/index.php?app=member_card&feiwa=mcard_info&uid='+data.id, function(d){
			                			if(d){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+MEMBER_SITE_URL+'/index.php?app=member_message&feiwa=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(MEMBER_SITE_URL+'/index.php?app=member_snsfriend&feiwa=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( "data_str = "+data_str);
			$.getJSON(MEMBER_SITE_URL+'/index.php?app=member_snsfriend&feiwa=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function mall_function(d){
			return ;
		}
		function circle_function(d){
			var rs = $('<dd class="ajax-info"></dd>');
			$.each(d,function(i, n){
				rs.append('<div class="rank-div" title="'+n.circle_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'"><a href="'+CIRCLE_SITE_URL+'/index.php?app=group&c_id='+n.circle_id+'" target="_blank">'+n.circle_name+'</a><i></i><em class="rank-em rank-'+n.cm_level+'">'+n.cm_level+'</em></div>');
			});
			return rs;
		}
		function shareshow_function(d){
			var rs = $('<dd class="ajax-info"></dd>');
            rs.append('<span class="ajax-info-shareshow">随心看：' + d.goods_count + '</span>');
            rs.append('<span class="ajax-info-shareshow">个人秀：' + d.personal_count + '</span>');
			return rs;
		}
	};
})(jQuery);
 ...
 * $("#region").feiwa_region({{src:'db'}});
 * 
 * 如需指定地区下拉显示层级，需指定show_deep参数，默认未限制
 * $("#region").feiwa_region({{show_deep:2}}); 这样最多只会显示2级联动
 * 
...
(function($) {
	$.fn.feiwa_region = function(options) {
		var $region = $(this);
		var settings = $.extend({}, {
			area_id: 0,
		...
		this.each(function() {
			...
			if ($inputArea.val() && /^\d+$/.test($inputArea.val())) {
				$.getJSON(SITEURL + "/index.php?app=index&feiwa=json_area_show&area_id=" + $inputArea.val() + "&callback=?", function(data) {
		...
		function loadArea($area, area_id) {
			if ($area && feiwa_a[area_id].length > 0) {
				var areas = [];
		...
		function loadAreaArray(callback) {
			if (typeof feiwa_a === "undefined") {
				$.getJSON(SITEURL + "/index.php?app=index&feiwa=json_area&src=" + settings.src + "&callback=?", function(data) {
					feiwa_a = data;
					callback()
				})
			} else {
...
function addcart(goods_id,quantity,callbackfunc) {
    ...
    var url = 'index.php?app=cart&feiwa=add';
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $("#rtoolbar_cartlist").load(MALL_SITE_URL + '/index.php?app=cart&feiwa=ajax_load&type=html');
            } else {
                alert(data.msg);
            }
        }
...
//      $('#hdSearchTab ul li').eq(0).attr('app','store_list');
//      $('#search_app').attr("value","store_list");
//      }
...
$(function(){
    ...
    if (app == "store_list"){
        $('#hdSearchTab ul li span').eq(0).html('店铺');
        $('#hdSearchTab ul li span').eq(1).html('商品');
        $('#hdSearchTab ul li span').eq(2).html('品牌');//
        $('#hdSearchTab ul li').eq(0).attr('app','store_list');
        $('#search_app').attr("value","store_list");
        }
    if (app == "brand"){
        $('#hdSearchTab ul li span').eq(0).html('品牌');
        $('#hdSearchTab ul li span').eq(1).html('商品');
        $('#hdSearchTab ul li span').eq(2).html('店铺');//
        $('#hdSearchTab ul li').eq(0).attr('app','brand');
        $('#search_app').attr("value","brand");
        }
    $('#hdSearchTab').hover(function(){
        $('#hdSearchTab ul li').eq(1).show();
        $('#hdSearchTab ul li i').addClass('over').removeClass('arrow');
        $('#hdSearchTab ul li').eq(2).show();
        $('#hdSearchTab ul li i').addClass('over').removeClass('arrow');
    },function(){
        $('#hdSearchTab ul li').eq(1).hide();
        $('#hdSearchTab ul li i').addClass('arrow').removeClass('over');
        $('#hdSearchTab ul li').eq(2).hide();
        $('#hdSearchTab ul li i').addClass('arrow').removeClass('over');
    });
    $('#hdSearchTab ul li').eq(1).click(function(){
        $(this).hide();
    ...
    $('#hdSearchTab ul li').eq(2).click(function(){
        $(this).hide();
        if($(this).find('span').html() == '品牌') {
