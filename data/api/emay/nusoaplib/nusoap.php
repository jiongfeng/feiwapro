	...
	function setCredentials($username, $password, $authtype = 'basic', $digestRequest = array(), $certRequest = array()) {
		...
		if ($authtype == 'basic') {
			...
			if (isset($digestRequest['nonce'])) {
				...
				// calculate the Digest hashes (calculate code based on digest implementation found at: http://www.rassoc.com/gregr/weblog/stories/2002/07/09/webServicesSecurityHttpDigestAuthenticationWithoutadtiveDirectory.html)
    ...
    */
    function webDescription(){
		...
		<style type="text/css">
		    ...
		    a,a:adtive  { color: charcoal; font-weight: bold; }
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					if (isset($opParts['input']['encodingStyle']) && $opParts['input']['encodingStyle'] != '') {
						$efeiwa_style = ' encodingStyle="' . $opParts['input']['encodingStyle'] . '"';
					} else {
						$efeiwa_style = '';
					}
					$binding_xml .= "\n" . '    <input><soap:body use="' . $opParts['input']['use'] . '" namespace="' . $opParts['input']['namespace'] . '"' . $efeiwa_style . '/></input>';
					if (isset($opParts['output']['encodingStyle']) && $opParts['output']['encodingStyle'] != '') {
						$efeiwa_style = ' encodingStyle="' . $opParts['output']['encodingStyle'] . '"';
					} else {
						$efeiwa_style = '';
					}
					$binding_xml .= "\n" . '    <output><soap:body use="' . $opParts['output']['use'] . '" namespace="' . $opParts['output']['namespace'] . '"' . $efeiwa_style . '/></output>';
					$binding_xml .= "\n" . '  </operation>';
			...
			if (is_array($parameters)) {
				...
				foreach ($parts as $name => $type) {
					...
					if ($parametersArrayType == 'arraySimple') {
						...
						$xml .= $this->serializeType($name, $type, $p, $use, $efeiwa_style);
					} elseif (isset($parameters[$name])) {
						$this->debug('calling serializeType w/named param');
						$xml .= $this->serializeType($name, $type, $parameters[$name], $use, $efeiwa_style);
					} else {
						// TODO: only send nillable
						$this->debug('calling serializeType w/null param');
						$xml .= $this->serializeType($name, $type, null, $use, $efeiwa_style);
					}
				}
			} else {
