$(function(){
    ...
    $('#goods_image').fileupload({
        dataType: 'json',
        url: SITEURL + '/index.php?app=store_goods_add&feiwa=image_upload&upload_type=uploadedfile',
    ...
    $('#add_album').fileupload({
        dataType: 'json',
        url: SITEURL+'/index.php?app=store_goods_add&feiwa=image_upload',
    ...
    // 商品属性
    attr_selected();
    $('select[feiwa_type="attr_select"]').change(function(){
        id = $(this).find('option:selected').attr('feiwa_type');
        name = $(this).attr('attr').replace(/__FEIWA__/g,id);
        $(this).attr('name',name);
    });
    ...
    // 商品所在地
    $("#region").feiwa_region({show_deep:2,tip_type:1});
    ...
    $('a[nctype="specAddSubmit"]').click(function(){
        ...
        $.getJSON(data_str.url, {gc_id : data_str.gc_id , sp_id : data_str.sp_id , name : _input.val()}, function(data){
            if (data.done) {
                _parent.before('<li><span nctype="input_checkbox"><input type="checkbox" name="sp_val[' + data_str.sp_id + '][' + data.value_id + ']" feiwa_type="' + data.value_id + '" value="' +_input.val()+ '" /></span><span nctype="pv_name">' + _input.val() + '</span></li>');
                _input.val('');
            }
        });
    });
    ...
    $('ul[nctype="brand_list"]').on('click', 'li', function(){
        ...
        $('.feiwast-brand-select > .feiwast-brand-select-container').hide();
    });
    //搜索品牌列表滚条绑定
    ...
    $('input[name="b_name"]').focus(function(){
        $('.feiwast-brand-select > .feiwast-brand-select-container').show();
    });
    ...
    $('div[nctype="mobile_pannel"]').on('click', '[nctype="mp_edit"]', function(){
        ...
        $(this).parents('.module:first').html('')
            ...
            .append('<div class="feiwast-mea-text" nctype="mea_txt"></div>')
            ...
            .append('<div class="button"><a class="feiwast-btn feiwast-btn-blue" nctype="meat_edit_submit" href="javascript:void(0);">确认</a><a class="feiwast-btn ml10" nctype="meat_edit_cancel" href="javascript:void(0);">取消</a></div>')
    ...
    $('a[nctype="mb_add_img"]').click(function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('index.php?app=store_album&feiwa=pic_list&item=mobile');
    });
    // 关闭 图片选择
    ...
    $('div[nctype="mobile_pannel"]').on('click', 'a[nctype="mp_rpl"]', function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('index.php?app=store_album&feiwa=pic_list&item=mobile&type=replace');
    });
    // 插图图片
...
function change_img_name(Obj){
     ...
     S.val(Obj.val());
     var V = $('tr[nctype="file_tr_'+S.attr('feiwa_type')+'"]');
     V.find('span[nctype="pv_name"]').html(Obj.val());
     V.find('input[type="file"]').attr('name', Obj.val());
}
// 商品属性
function attr_selected(){
    $('select[feiwa_type="attr_select"] option:selected').each(function(){
        id = $(this).attr('feiwa_type');
        name = $(this).parents('select').attr('attr').replace(/__FEIWA__/g,id);
