$(function(){
    ...
    //展示和隐藏评论列表
	$("[feiwa_type='sd_commentbtn']").live('click',function(){
		var $this = $(this);
		$.get('index.php?app=index&feiwa=login', function(result){
		    if(result=='0'){
				...
				if($('#tracereply_'+data.txtid).css("display")=='none'){
					//加载评论列表
			        $("#tracereply_"+data.txtid).load('index.php?app=store_snshome&feiwa=commenttop&id='+data.txtid);
	...
	//评论提交
	$("[feiwa_type='scommentbtn']").live('click',function(){
		var data = $(this).attr('data-param');
        eval("data = "+data);
	...
	//删除评论
	$("[feiwa_type='scomment_del']").live('click',function(){
		var obj = $(this);
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
			var data_str = $(obj).attr('data-param');
	        eval("data_str = "+data_str);
	        ajax_get_confirm('','index.php?app=store_snshome&feiwa=delcomment&scid='+data_str.scid+'&stid='+data_str.stid);
			return false;
		});
	});
	...
	//展示和隐藏转发表单
	$("[feiwa_type='sd_forwardbtn']").live('click',function(){
		var $this = $(this);
		$.get('index.php?app=index&feiwa=login', function(result){
		    if(result=='0'){
		    	login_dialog();
	...
	//转发提交
	$("[feiwa_type='s_forwardbtn']").live('click',function(){
		var data = $(this).attr('data-param');
		...
		if(form.valid()){
			...
			if(!isNaN(cookienum) && cookienum >= MAX_RECORDNUM){
			    if (seccode.css('display') == 'none') {
	                ...
	                seccode.find("[name='codeimage']").attr('src','index.php?app=seccode&feiwa=makecode&nchash='+nchash+'&t=' + Math.random());
    ...
    //删除动态
	$("[feiwa_type='sd_del']").live('click',function(){
		var data_str = $(this).attr('data-param');
        eval("data_str = "+data_str);
        var url = "index.php?app=store_snshome&feiwa=deltrace&id="+data_str.txtid;
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
			ajaxget(url);
			return false;
		});
