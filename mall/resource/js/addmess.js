...
$(function(){
	...
	check_tuwen(1);
	//var url = "/biz.php?ctl=wechat&app=showmess";
	var url = "index.php?app=seller_wechat&feiwa=showmess";
...
function zsc_editimg(num){
	...
	if(0==zsc_count){
		$('#zsc_titles').val($('#zsc_titles0').html());
		$('#zsc_urls').val($('#zsc_urls0').val());
		if('/mall/resource/keyword/images/bigimg.jpg'==$('#zsc_imgs0').attr('src')){
			$('#zsc_imgurls').val();
		}else{
		...
		if('/mall/resource/keyword/images/smallimg.jpg'!=ssurl){
...
function btnsave(num,count){
	...
	if(ktype==1){
		...
		//var url = "/biz.php?ctl=wechat&app=addmess";
		var url = "index.php?app=seller_wechat&feiwa=addmess";
