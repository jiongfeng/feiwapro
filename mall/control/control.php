...
class Control{
    ...
    protected function checkLogin(){
        if ($_SESSION['is_login'] !== '1'){
            if (trim($_GET['feiwa']) == 'favoritegoods' || trim($_GET['feiwa']) == 'favoritestore'){
    ...
    private function _getMenuList() {
        $menu_list = array(
            'trade' => array('name' => '交易中心', 'child' => array(
                'member_order'      => array('name' => '实物交易订单', 'url' => urlMall('member_order', 'index')),
                'member_vr_order'   => array('name' => '虚拟兑码订单', 'url' => urlMall('member_vr_order', 'index')),
                'member_evaluate'   => array('name' => '交易评价/晒单', 'url' => urlMall('member_evaluate', 'list')),
                'member_appoint'    => array('name' => '预约/到货通知', 'url' => urlMall('member_appoint', 'list'))
            )),
            'follow' => array('name' => '关注中心', 'child' => array(
                'member_favorite_goods' => array('name' => '商品收藏', 'url' => urlMall('member_favorite_goods', 'index')),
                'member_favorite_store' => array('name' => '店铺收藏', 'url' => urlMall('member_favorite_store', 'index')),
                'member_goodsbrowse'   => array('name' => '我的足迹', 'url' => urlMall('member_goodsbrowse', 'list'))
            )),
            'client' => array('name' => '客户服务', 'child' => array(
                'member_refund'     => array('name' => '退款及退货', 'url' => urlMall('member_refund', 'index')),
                'member_complain'   => array('name' => '交易投诉', 'url' => urlMall('member_complain', 'index')),
                'member_consult'    => array('name' => '商品咨询', 'url' => urlMall('member_consult', 'my_consult')),
                'member_inform'     => array('name' => '违规举报', 'url' => urlMall('member_inform', 'index')),
                'member_mallconsult'=> array('name' => '平台客服', 'url' => urlMall('member_mallconsult', 'index'))
            )),
...
class BaseSNSControl extends Control {
    ...
    protected function formatDate($time){
        ...
        if ($reference_date_time == $handle_date_time){
            ...
            $month_text = Language::get('feiwa_month');
            $day_text = Language::get('feiwa_day');
    ...
    public function get_member_info() {
        ...
        if(empty($member_info)){
            showMessage(L('wrong_argument'), 'index.php?app=member_snshome', '', 'error');
        }
        //粉丝数
    ...
    private function check_relation(){
        ...
        if ($this->master_id <= 0){
            if ($_SESSION['is_login'] == 1){
                ...
                @header("location: " . urlLogin('login', 'index', array('ref_url' => urlMall('member_snshome'))));
            }
        }
...
class BaseStoreControl extends Control {
    ...
    public function __construct(){
        ...
        if($store_id <= 0) {
            showMessage(L('feiwa_store_close'), '', '', 'error');
        }
    ...
    protected function outputStoreInfo($store_info, $goods_info = null) {
        if (!$this->store_decoration_only) {
            ...
            // 自营店设置“显示商城相关数据”
            if ($goods_info && $store_info['is_own_mall'] && $store_info['left_bar_type'] == 2) {
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        ...
        Tpl::output('nav_list', rkcache('nav',true));
        if ($_GET['app'] !== 'seller_login') {

            if(empty($_SESSION['seller_id'])) {
                @header('location: index.php?app=seller_login&feiwa=show_login');die;
            }
            ...
            if (checkPlatformStore()) {
                $this->store_grade = array(
                    ...
                    // see also store_settingControl.themeFeiwa()
            ...
            if ($_SESSION['seller_is_admin'] !== 1 && $_GET['app'] !== 'seller_center' && $_GET['app'] !== 'seller_logout') {
                if (!in_array($_GET['app'], $_SESSION['seller_limits'])) {
    ...
    protected function recordSellerLog($content = '', $state = 1){
        ...
        $seller_info['log_url'] = $_GET['app'].'&'.$_GET['feiwa'];
        ...
        if (intval($is_admin) !== 1) {
            ...
            foreach ($menu_list as $key => $value) {
                foreach ($value['child'] as $child_key => $child_value) {
                    if (!in_array($child_value['app'], $limits)) {
    ...
    private function _getCurrentMenu($seller_function_list) {
        $current_menu = $seller_function_list[$_GET['app']];
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                array('name' => '商品发布', 'app'=>'store_goods_add', 'feiwa'=>'index'),
			    array('name' => '出售中的商品', 'app'=>'store_goods_online', 'feiwa'=>'index'),
                array('name' => '仓库中的商品', 'app'=>'store_goods_offline', 'feiwa'=>'index'),
                array('name' => '预约/到货通知', 'app' => 'store_appoint', 'feiwa' => 'index'),
                array('name' => '关联版式', 'app'=>'store_plate', 'feiwa'=>'index'),
                array('name' => '商品规格', 'app' => 'store_spec', 'feiwa' => 'index'),
                array('name' => '图片空间', 'app'=>'store_album', 'feiwa'=>'album_cate'),
            )),
            'order' => array('name' => '订单', 'child' => array(
                array('name' => '实物交易订单', 'app'=>'store_order', 'feiwa'=>'index'),
                array('name' => '虚拟兑码订单', 'app'=>'store_vr_order', 'feiwa'=>'index'),
                array('name' => '发货', 'app'=>'store_deliver', 'feiwa'=>'index'),
                array('name' => '发货设置', 'app'=>'store_deliver_set', 'feiwa'=>'daddress_list'),
                array('name' => '运单模板', 'app'=>'store_waybill', 'feiwa'=>'waybill_manage'),
                array('name' => '评价管理', 'app'=>'store_evaluate', 'feiwa'=>'list'),
                array('name' => '物流工具', 'app'=>'store_transport', 'feiwa'=>'index'),
				array('name' => '来单提醒', 'app'=>'order_call', 'feiwa'=>'index'),
            )),
            'promotion' => array('name' => '促销', 'child' => array(
                array('name' => '团购管理', 'app'=>'store_groupbuy', 'feiwa'=>'index'),
                array('name' => '加价购', 'app'=>'store_promotion_cou', 'feiwa'=>'cou_list'),
                array('name' => '限时折扣', 'app'=>'store_promotion_xianshi', 'feiwa'=>'xianshi_list'),
                array('name' => '满即送', 'app'=>'store_promotion_mansong', 'feiwa'=>'mansong_list'),
                array('name' => '优惠套装', 'app'=>'store_promotion_bundling', 'feiwa'=>'bundling_list'),
                array('name' => '推荐展位', 'app' => 'store_promotion_booth', 'feiwa' => 'booth_goods_list'),
                array('name' => '预售商品', 'app' => 'store_promotion_book', 'feiwa' => 'index'),
                array('name' => 'F码商品', 'app' => 'store_promotion_fcode', 'feiwa' => 'index'),
                array('name' => '推荐组合', 'app' => 'store_promotion_combo', 'feiwa' => 'index'),
                array('name' => '手机专享', 'app' => 'store_promotion_sole', 'feiwa' => 'index'),
                array('name' => '代金券管理', 'app'=>'store_voucher', 'feiwa'=>'templatelist'),
                array('name' => '活动管理', 'app'=>'store_activity', 'feiwa'=>'store_activity'),
            )),
            'store' => array('name' => '店铺', 'child' => array(
                array('name' => '店铺设置', 'app'=>'store_setting', 'feiwa'=>'store_setting'),
                array('name' => '店铺装修', 'app'=>'store_decoration', 'feiwa'=>'decoration_setting'),
                array('name' => '店铺导航', 'app'=>'store_navigation', 'feiwa'=>'navigation_list'),
                array('name' => '店铺动态', 'app'=>'store_sns', 'feiwa'=>'index'),
                array('name' => '店铺信息', 'app'=>'store_info', 'feiwa'=>'bind_class'),
                array('name' => '店铺分类', 'app'=>'store_goods_class', 'feiwa'=>'index'),
                array('name' => '品牌申请', 'app'=>'store_brand', 'feiwa'=>'brand_list'),
                array('name' => '供货商', 'app'=>'store_supplier', 'feiwa'=>'sup_list'),
                array('name' => '实体店铺', 'app'=>'store_map', 'feiwa'=>'index'),
                array('name' => '消费者保障服务', 'app'=>'store_contract', 'feiwa'=>'index'),
            )),
            'consult' => array('name' => '售后', 'child' => array(
                array('name' => '咨询管理', 'app'=>'store_consult', 'feiwa'=>'consult_list'),
                array('name' => '投诉管理', 'app'=>'store_complain', 'feiwa'=>'list'),
                array('name' => '退款记录', 'app'=>'store_refund', 'feiwa'=>'index'),
                array('name' => '退货记录', 'app'=>'store_return', 'feiwa'=>'index'),
            )),
            'statistics' => array('name' => '运营', 'child' => array(
                array('name' => '店铺概况', 'app'=>'statistics_general', 'feiwa'=>'general'),
                array('name' => '商品分析', 'app'=>'statistics_goods', 'feiwa'=>'goodslist'),
                array('name' => '运营报告', 'app'=>'statistics_sale', 'feiwa'=>'sale'),
                array('name' => '行业分析', 'app'=>'statistics_industry', 'feiwa'=>'hot'),
                array('name' => '流量统计', 'app'=>'statistics_flow', 'feiwa'=>'storeflow'),
                array('name' => '实物结算', 'app'=>'store_bill', 'feiwa'=>'index'),
                array('name' => '虚拟结算', 'app'=>'store_vr_bill', 'feiwa'=>'index'),
				array('name' => '淘宝在线采集', 'app'=>'taobao_caiji', 'feiwa'=>'batchco'),
				array('name' => '淘宝CSV', 'app'=>'taobao_import', 'feiwa'=>'index'),
            )),
            'message' => array('name' => '客服', 'child' => array(
                array('name' => '客服设置', 'app'=>'store_callcenter', 'feiwa'=>'index'),
                array('name' => '系统消息', 'app'=>'store_msg', 'feiwa'=>'index'),
                array('name' => '聊天记录查询', 'app'=>'store_im', 'feiwa'=>'index'),
            )),
            'account' => array('name' => '账号', 'child' => array(
                array('name' => '账号列表', 'app'=>'store_account', 'feiwa'=>'account_list'),
                array('name' => '账号组', 'app'=>'store_account_group', 'feiwa'=>'group_list'),
                array('name' => '账号日志', 'app'=>'seller_log', 'feiwa'=>'log_list'),
                array('name' => '店铺消费', 'app'=>'store_cost', 'feiwa'=>'cost_list'),
                array('name' => '门店账号', 'app'=>'store_chain', 'feiwa'=>'index'),
            ))
        );
        return $menu_list;
    }
    ...
    private function _getSellerFunctionList($menu_list) {
        ...
        foreach ($menu_list as $key => $menu_value) {
            foreach ($menu_value['child'] as $submenu_value) {
                $format_menu[$submenu_value['app']] = array(
                    'model' => $key,
                    'model_name' => $menu_value['name'],
                    'name' => $submenu_value['name'],
                    'app' => $submenu_value['app'],
                    'feiwa' => $submenu_value['feiwa'],
                );
            }
        }
        return $format_menu;
    ...
    public function storeAutoShare($data, $type) {
        ...
        if ($auto_setting['sauto_' . $type] == 1) {
            ...
            if ($auto_setting['sauto_' . $type . 'title'] != '') {
                ...
                $auto_title = 'feiwa_store_auto_share_' . $type . rand(1, 5);
 ...
 * 积分中心control父类
 */
class BasePointMallControl extends Control {
    ...
    public function __construct(){
        ...
        //判断系统是否开启积分和积分中心功能
        if (C('points_isuse') != 1 || C('pointmall_isuse') != 1){
            showMessage(Language::get('pointmall_unavailable'),urlMall('index','index'),'html','error');
        }
        Tpl::output('index_sign','pointmall');
    }
    /**
     * 获得积分中心会员信息包括会员名、ID、会员头像、会员等级、经验值、等级进度、积分、已领代金券、已兑换礼品、礼品购物车
     */
    public function pointmallMInfo($is_return = false){
        if($_SESSION['is_login'] == '1'){
            $model_member = Model('member');
            if (!$this->member_info){
                //查询会员信息
