 ...
 * @link       联系电话：0539-889333 客服QQ：2116198029 欢迎加入MALL WWI.COM
 */
...
class brandControl extends BaseHomeControl {
    public function indexFeiWa(){
        ...
        $gc_id['brand_name'] = array('like', '%' . $_GET['keyword'] . '%');

        //获得品牌列表
        $model = Model();
        //分页 by liuxuexin 
        $brand_m_list = $model->table('brand')->where($gc_id)->order('brand_sort asc')->page(24)->select();
    ...
    public function listFeiWa(){
        ...
        // 得到排序方式
        $order = 'is_own_mall desc,goods_id desc';
        if (!empty($_GET['key'])) {
