...
class store_promotion_xianshiControl extends BaseSellerControl {
    ...
    const LINK_XIANSHI_LIST = 'index.php?app=store_promotion_xianshi&feiwa=xianshi_list';
    const LINK_XIANSHI_MANAGE = 'index.php?app=store_promotion_xianshi&feiwa=xianshi_manage&xianshi_id=';
    ...
    public function __construct() {
        ...
        if (intval(C('promotion_allow')) !== 1){
            showMessage(Language::get('promotion_unavailable'),'index.php?app=store','','error');
        }
     ...
     * 添加限时折扣活动
     **/
    public function xianshi_addFeiwa() {
        if (checkPlatformStore()) {
            Tpl::output('isOwnMall', true);
        } else {
            $model_xianshi_quota = Model('p_xianshi_quota');
		...
		$xianshi_class = Model('p_xianshi_class')->getList();
        Tpl::output('xianshi_class', $xianshi_class);
     ...
     * 保存添加的限时折扣活动
     **/
    public function xianshi_saveFeiwa() {
        //验证输入
		...
		$param['xianshi_image'] = $_POST['xianshi_image5'];
        $param['xianshi_image1'] = $_POST['xianshi_image1'];
		$param['xianshi_image2'] = $_POST['xianshi_image2'];
		$param['class_id'] =intval($_POST['sc_id']);
		$param['xianshi_intro'] = $_POST['xianshi_intro'];
     ...
     * 编辑限时折扣活动
     **/
    public function xianshi_editFeiwa() {
        $model_xianshi = Model('p_xianshi');
     ...
     * 编辑保存限时折扣活动
     **/
    public function xianshi_edit_saveFeiwa() {
        ...
        if($result && $result) {
            ...
            showDialog(Language::get('feiwa_common_op_succ'),self::LINK_XIANSHI_LIST,'succ','',3);
        }else {
            showDialog(Language::get('feiwa_common_op_fail'));
        }
    }
    ...
    public function xianshi_delFeiwa() {
        ...
        if($result) {
            ...
            showDialog(L('feiwa_common_op_succ'), urlMall('store_promotion_xianshi', 'xianshi_list'), 'succ');
        } else {
     ...
     * 限时折扣活动管理
     **/
    public function xianshi_manageFeiwa() {
        $model_xianshi = Model('p_xianshi');
        $model_xianshi_goods = Model('p_xianshi_goods');
     ...
     * 限时折扣套餐购买
     **/
    public function xianshi_quota_addFeiwa() {
        //输出导航
     ...
     * 限时折扣套餐购买保存
     **/
    public function xianshi_quota_add_saveFeiwa() {
        ...
        $this->recordSellerLog('购买'.$xianshi_quota_quantity.'份限时折扣套餐，单价'.$current_price.$lang['feiwa_yuan']);
     ...
     * 选择活动商品
     **/
    public function goods_selectFeiwa() {
     ...
     * 限时折扣商品添加
     **/
    public function xianshi_goods_addFeiwa() {
     ...
     * 限时折扣商品价格修改
     **/
    public function xianshi_goods_price_editFeiwa() {
        ...
        if($result) {
            ...
            $data['result'] = false;
            $data['message'] = L('feiwa_common_op_succ');
        }
        echo json_encode($data);die;
    }
     ...
     * 限时折扣商品删除
     **/
    public function xianshi_goods_deleteFeiwa() {
        $model_xianshi_goods = Model('p_xianshi_goods');
        $model_xianshi = Model('p_xianshi');
   ...
   /**
    ...
    public function image_uploadFeiwa() {
        if(!empty($_POST['old_xianshi_image'])) {
            $this->_image_del($_POST['old_xianshi_image']);
        }
        $this->_image_upload('xianshi_image');
    }
    ...
    private function _image_upload($file) {
        ...
        if(!empty($_FILES[$file]['name'])) {
            ...
            $uploaddir = ATTACH_PATH.DS.'ttm'.DS.$_SESSION['store_id'].DS;
            ...
            if($result) {
                ...
                $data['file_url'] = xsthumb($upload->file_name);
            } else {
                $data['result'] = false;
    ...
    private function _image_del($image_name) {
        ...
        $image_path = BASE_UPLOAD_PATH.DS.ATTACH_TTM.DS.$store_id.DS;
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'xianshi_list','menu_name'=>Language::get('promotion_active_list'),'menu_url'=>'index.php?app=store_promotion_xianshi&feiwa=xianshi_list'),
        );
        switch ($menu_key){
            case 'xianshi_add':
                $menu_array[] = array('menu_key'=>'xianshi_add','menu_name'=>Language::get('promotion_join_active'),'menu_url'=>'index.php?app=store_promotion_xianshi&feiwa=xianshi_add');
                break;
            case 'xianshi_edit':
                $menu_array[] = array('menu_key'=>'xianshi_edit','menu_name'=>'编辑活动','menu_url'=>'javascript:;');
                break;
            case 'xianshi_quota_add':
                $menu_array[] = array('menu_key'=>'xianshi_quota_add','menu_name'=>Language::get('promotion_buy_product'),'menu_url'=>'index.php?app=store_promotion_xianshi&feiwa=xianshi_quota_add');
                break;
            case 'xianshi_manage':
                $menu_array[] = array('menu_key'=>'xianshi_manage','menu_name'=>Language::get('promotion_goods_manage'),'menu_url'=>'index.php?app=store_promotion_xianshi&feiwa=xianshi_manage&xianshi_id='.$_GET['xianshi_id']);
                break;
        }
