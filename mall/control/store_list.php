...
class store_listControl extends BaseHomeControl {
	...
	public function indexFeiwa(){
		...
		if (intval($_GET['cate_id']) > 0){
			$nav_link[1]['link'] = 'index.php?app=mall_search';
			...
			if ($nav['sc_parent_id'] > 0){
				...
				$nav_link[] = array(
					'title'=>$tmp['sc_name'],
					'link'=>"index.php?app=store_list&cate_id=".$nav['sc_parent_id']
				);
			}
			//存入当前级
			$nav_link[] = array(
		...
		Tpl::output('purl', urlMall($purl['app'], $purl['feiwa'], $purl['param']));

		//SEO
		Model('seo')->type('index')->show();
		Tpl::output('html_title',(empty($_GET['keyword']) ? '' : $_GET['keyword'].' - ').C('site_name').$lang['feiwa_common_search']);
		
		$boothgoods_list = Model('p_booth')->getBoothGoodsList( 'goods_id', 0, 5, 'rand()');
	...
	private function full_search($search_txt){
		...
		$res = $cl->Query($search_txt, $conf['index_mall']);
		if ($res){
	...
	function getParam() {
	    ...
	    $purl['app'] = $param['app'];
	    unset($param['app']);
	    $purl['feiwa'] = $param['feiwa'];
	    unset($param['feiwa']); unset($param['curpage']);
	    $purl['param'] = $param;
	    return $purl;
	}
}
