...
<div class="ncm-flow-layout">
  <div class="ncm-flow-container">
    ...
    <form id="evalform" method="post" action="index.php?app=member_evaluate&feiwa=<?php echo $_GET['feiwa'];?>&order_id=<?php echo $_GET['order_id'];?>">
      ...
      <table class="ncm-default-table mb30">
        <tbody>
          ...
          <tr class="bd-line">
            <td class="w20"></td>
            <td class="pic-mode w200"><div class="pic-thumb"><a href="index.php?app=goods&goods_id=<?php echo $goods['goods_id']; ?>" target="_blank"><img src="<?php echo $goods['goods_image_url']; ?>"/></a></div>
              <dl class="goods-name">
                <dt><a href="index.php?app=goods&goods_id=<?php echo $goods['goods_id'];?>" target="_blank" title="<?php echo $goods['goods_name'];?>"><?php echo $goods['goods_name'];?></a></dt>
                <dd title="<?php echo $goods['goods_spec'];?>"><?php echo $goods['goods_spec'];?></dd>
              </dl></td>
            <td valign="top" class="tl"><div class="ncgeval">商品评分：
                <div class="raty vm">
                  <input nctype="score" name="goods[<?php if ($_GET['feiwa'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][score]" type="hidden">
                </div>
              </div>
              <textarea name="goods[<?php if ($_GET['feiwa'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][comment]" cols="150" class="w450 mt10 mb10 h40" placeholder="请输入要评价的内容，不要超过150个字符。"></textarea>
              <div class="show-pic">
                <?php if ($_GET['feiwa'] != 'add_vr') {?>
                ...
                <input type="checkbox" class="checkbox vm" name="goods[<?php if ($_GET['feiwa'] != 'add_vr') { echo $goods['rec_id'];} else { echo $goods['goods_id'];}?>][anony]">
      ...
      <?php if (!$output['store_info']['is_own_mall'] && $_GET['feiwa'] != 'add_vr') { ?>
      <div class="tabmenu">
        <ul class="tab">
      ...
      <div class="ncm-default-form">
        <?php if (!$output['store_info']['is_own_mall'] && $_GET['feiwa'] != 'add_vr') { ?>
        <dl>
          <dt><?php echo $lang['member_evaluation_evalstore_type_1'].$lang['feiwa_colon'];?></dt>
          <dd>
            <div class="raty-x2">
        ...
        <dl>
          <dt><?php echo $lang['member_evaluation_evalstore_type_2'].$lang['feiwa_colon'];?></dt>
          <dd>
            <div class="raty-x2">
        ...
        <dl>
          <dt><?php echo $lang['member_evaluation_evalstore_type_3'].$lang['feiwa_colon'];?></dt>
          <dd>
            <div class="raty-x2">
...
$(function(){
    ...
    <?php if(!empty($output['order_goods'])){?>
    <?php foreach($output['order_goods'] as $goods){?>
    <?php $param_id = $_GET['feiwa'] != 'add_vr' ? $goods['rec_id'] : $goods['goods_id'];?>
    // 图片上传
    $('#file<?php echo $param_id;?>').fileupload({
        dataType: 'json',
        url: '<?php echo urlMall('sns_album', 'swfupload');?>',
        formData: '',
        add: function (e,data) {
    ...
    var album_pic_del = function(file_id) {
        var del_url = "<?php echo urlMall('sns_album', 'album_pic_del');?>";
        del_url += '&id=' + file_id;
        $.get(del_url);
    }
    <?php }?>
