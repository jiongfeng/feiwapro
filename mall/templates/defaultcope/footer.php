...
function fade() {
	$("img[rel='lazy']").each(function () {
		...
		if ($scroTop.top <= $(window).scrollTop() + $(window).height()) {
			$(this).hide();
			$(this).attr("src", $(this).attr("feiwa-url"));
			$(this).removeAttr("rel");
...
<?php echo $output['setting_config']['feiwa_version'];?>-<?php echo feiwa_by_version;?>
			...
			<p><?php echo html_entity_decode($output['setting_config']['statistics_code'],ENT_QUOTES); ?></p></div></div>
<?php if (C('debug') == 1){?>
	<div id="think_page_trace" class="trace">
		<fieldset id="querybox"><legend><?php echo $lang['feiwa_debug_trace_title'];?></legend><div> 
