...
<form method="get" action="index.php">
  <input type="hidden" name="app" value="store_refund" />
  ...
  <tbody>
    ...
    <tr class="bd-line" >
        ...
        <td class="w50"><div class="pic-thumb">
            <a href="<?php echo urlMall('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank">
        ...
        <dd><?php echo $lang['refund_order_refundsn'].$lang['feiwa_colon'];?><?php echo $val['refund_sn']; ?></dd></dl></td>
		    <?php } else { ?>
      ...
      <td class="nscs-table-handle"><?php if ($val['seller_state'] == 1) { ?>
    	<span><a href="index.php?app=store_refund&feiwa=edit&refund_id=<?php echo $val['refund_id']; ?>" class="btn-bluejeans"><i class="icon-edit"></i><p>处理</p></a></span>
    	<?php } else { ?>
    	<span><a href="index.php?app=store_refund&feiwa=view&refund_id=<?php echo $val['refund_id']; ?>" class="btn-bittersweet"><i class="icon-eye-open"></i><p><?php echo $lang['feiwa_view'];?></p></a></span><?php } ?>
       </td>
    </tr>
    <?php } ?>
    <?php } else { ?>
