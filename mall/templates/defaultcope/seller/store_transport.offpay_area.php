...
.area-list {display:inline-block;width:200px;padding:5px 0px;}
  ...
  <form id="area_form" method="post">
    ...
    <input type="hidden" name="app" value="store_transport" />
    <input type="hidden" name="feiwa" value="offpay_area" />
    <input type="hidden" name="county" id="county" value="" />

    <table class="feiwast-default-table order" id="table_area_box">
      <tbody>
...
<?php foreach ($areas['children'][$provinceId] as $cityId) { ?>
            <div class="area-list">
              <label>
                <input type="checkbox" feiwa_province="<?php echo $provinceId; ?>"<?php if ($output['city_checked_array'][$cityId]) echo ' checked="checked"';?> value="<?php echo $cityId; ?>" name="city[]" />
              ...
              <a city_id="<?php echo $cityId; ?>" feiwa_title="<?php echo $areas['name'][$cityId]; ?>" province_id="<?php echo $provinceId; ?>" feiwa_type="edit" href="javascript:void(0);" title="选择下级地区"><i class="icon-pencil"></i></a>
            </div>
<?php } ?>
<?php } ?>
          </td>
...
var feiwa_a = $.parseJSON('<?php echo $output['area_array_json']; ?>');
...
$(function(){
    ...
    $('input[name="province[]"]').on('click',function(){
        if ($(this).attr('checked') == 'checked'){
            $('input[feiwa_province="' + $(this).val() + '"]').each(function(){
                $(this).attr('checked','checked');
                if (typeof feiwa_a[$(this).val()] == 'object') {
                    county_array = feiwa_a[$(this).val()];
                } else {
    ...
    //点击编辑事件
    $('a[feiwa_type="edit"]').on('click',function(){
            if (typeof CUR_COUNTY[$(this).attr('city_id')] == 'object'){
                cur_county = CUR_COUNTY[$(this).attr('city_id')];
            }else{
                cur_county = new Array();
            }
            var province_array = feiwa_a[$(this).attr('city_id')];
            if (typeof feiwa_a[$(this).attr('city_id')] == 'object'){
                county_array = feiwa_a[$(this).attr('city_id')];
            }else{
                county_array = new Array();
            }
            ...
            html_form('select_county', '选择 '+ $(this).attr('feiwa_title') +' 子地区', county_html, 500,1);
    });
    ...
    $('input[name="city[]"]').on('click',function(){
        if ($(this).attr('checked')) {
            ...
            count = county_array.length;
            if ($('input[feiwa_province="'+$(this).attr('feiwa_province')+'"]').size() == $('input[feiwa_province="'+$(this).attr('feiwa_province')+'"]:checked').size()) {
                $('input[value="'+$(this).attr('feiwa_province')+'"]').attr('checked',true);
            } else {
                $('input[value="'+$(this).attr('feiwa_province')+'"]').attr('checked',false);
            }
        } else {
            CUR_COUNTY[$(this).val()] = undefined;
            count = 0;
    ...
    $('body').on('click','#county_submit',function(){
        ...
        if ($('input[feiwa_province="'+cur_td.attr('province_id')+'"]').size() == $('input[feiwa_province="'+cur_td.attr('province_id')+'"]:checked').size()) {
