...
<div class="alert alert-block mt10 mb10">
      <?php if(!empty($output['current_quota'])) { ?>
      <strong>套餐过期时间<?php echo $lang['feiwa_colon'];?></strong><strong style="color: #F00;"><?php echo date('Y-m-d H:i:s', $output['current_quota']['quota_endtime']);?></strong>
      <?php } else { ?>
...
<form method="get">
	<table class="search-form">
		...
		<input type="hidden" id='feiwa' name='feiwa' value='templatelist' />
		<tr>
			<td>&nbsp;</td>
			...
			<th><?php echo $lang['feiwa_status'];?></th>
			<td class="w120"><select class="w80" name="select_state">
					<option value="0"
						<?php if (!$_GET['select_state'] == '0'){ echo 'selected=true';}?>><?php echo $lang['feiwa_please_choose'];?></option>
			...
			<td class="w120"><select class="w80" name="gettype_sel">
					<option value=""
						<?php if (!$_GET['gettype_sel']){ echo 'selected=true';}?>><?php echo $lang['feiwa_please_choose'];?></option>
			...
			<td class="tc w70"><label class="submit-border"><input type="submit"
					class="submit" value="<?php echo $lang['feiwa_search'];?>" /></label></td>
...
<table class="feiwast-default-table">
	<thead>
		<tr>
			...
			<th class="w60"><?php echo $lang['feiwa_status'];?></th>
      ...
      <tr class="bd-line">
        	...
        	<?php if($val['voucher_t_state']==$output['templatestate_arr']['usable'][0] && (!$val['voucher_t_giveout']) && (!$val['voucher_t_isbuild'])) {//代金券模板有效并且没有领取时可以编辑?>
        		<span> <a class="btn-bluejeans"
					href="index.php?app=store_voucher&feiwa=templateedit&tid=<?php echo $val['voucher_t_id'];?>">
						<i class="icon-edit"></i>
						<p>编辑</p>
				</a>
			</span>
        	...
        	<!-- 代金券详细 -->
        	<span>
        	   <a class="btn-bluejeans" href="index.php?app=store_voucher&feiwa=templateinfo&tid=<?php echo $val['voucher_t_id'];?>"><i class="icon-th-list"></i><p>详细</p></a>
			</span>
        	<?php if ((!$val['voucher_t_giveout']) && (!$val['voucher_t_isbuild'])){//该模板没有发放过代金券时可以删除?>
        	<span> <a class="btn-grapefruit" href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['feiwa_ensure_del'];?>','index.php?app=store_voucher&feiwa=templatedel&tid=<?php echo $val['voucher_t_id'];?>');"><i class="icon-trash"></i><p>删除</p></a></span>
        	<?php }?>
        </td>
		</tr>
      <?php }?>
