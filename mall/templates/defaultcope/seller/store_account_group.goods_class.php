...
<div class="feiwast-account-container">
  ...
  <ul class="feiwast-account-container-list">
    ...
    <li>
      ...
      )</span> <a cate_id="<?php echo $key2; ?>" feiwa_title="<?php echo $value2['gc_name']; ?>" parent_id="<?php echo $key; ?>" feiwa_type="edit" href="javascript:void(0);" title="选择下级分类"><i class="icon-pencil"></i></a> 
      <!-- 三级分类循环到隐藏域 -->
...
$(document).ready(function(){
    ...
    $('input[deep_type="2"]').on('click',function(){
        if ($(this).attr('checked') == 'checked') {
            ...
            //父级选框是否也选中功能注释
//             tmp = $(this).parents('.feiwast-account-container').find('input[deep_type="2"]');
...
//         	$('#'+$(this).attr('parent_id')).prop('checked',false);
	...
	$('a[feiwa_type="edit"]').on('click',function(){
        ...
        html_form('select_cate', '选择 '+ $(this).attr('feiwa_title') +' 子分类', cate3_html, 600,1);
	});
	...
	$('body').on('click','#cate_3_submit', function(){
		...
		//父级选框是否也选中功能注释
//         tmp = $('#'+cur_edit_id).parents('.feiwast-account-container').find('input[deep_type="2"]');
//         if (tmp.size() == tmp.filter(':checked').size()){
...
//         }
    ...
    $('[nctype="btn_select_module_class"]').on('click', function() {
        if($(this).prop('checked')) {
            ...
            $(this).parents('.feiwast-account-container').find('span[id^="count_"]').each(function(){
                $(this).html("("+$(this).attr('childcount')+")");
            });
        } else {
            $(this).parents('.feiwast-account-container').find('input:checkbox').prop('checked', false);
            $(this).parents('.feiwast-account-container').find('span[id^="count_"]').html('(0)');
            $('#gc_select_all').prop('checked',false);
        }
    });
});
