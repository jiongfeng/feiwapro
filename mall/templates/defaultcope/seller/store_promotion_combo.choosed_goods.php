...
<form method="post" id="goods_combo" action="<?php echo urlMall('store_promotion_combo', 'save_combo');?>">
  <input type="hidden" name="form_submit" value="ok">
  <input type="hidden" name="goods_id" value="<?php echo $output['goods_info']['goods_id'];?>">
  <div class="feiwast-form-goods-combo-default" data-gid="<?php echo $output['goods_info']['goods_id'];?>">
  ...
  <div nctype="feiwast-goods-combo" class="feiwast-goods-combo" id="ncscGoodsCombo">
    <?php if (!empty($output['combo_list'])) {$i = 0;?>
    <?php foreach ($output['combo_list'] as $key => $value) {?>
    <div class="feiwast-form-goods-combo">
      <div class="combo-title">分类名称
        <input type="text" name="class[s<?php echo $i;?>]" value="<?php echo $value['name'];?>" >
        <a href="javascript:void(0);" nctype="feiwast-goods-combo-del" class="ncbtn ncbtn-grapefruit"><i class="icon-trash"></i>删除</a>
      </div>
