...
<div class="tabmenu">
  ...
  <a href="<?php echo urlMall('store_vr_order', 'exchange');?>" class="ncbtn ncbtn-bittersweet"><i class="icon-edit"></i>兑换兑换码</a> </div>
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="app" value="store_vr_order" />
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20"><span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['feiwa_colon'];?><em><?php echo $order['order_sn']; ?></em>
        ...
        <?php }?>
        </span> <span><?php echo $lang['store_order_add_time'].$lang['feiwa_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$order['add_time']); ?></em></span> </th>
    </tr>
    <tr>
      <td class="bdl"></td>
      <td class="w70"><div class="feiwast-goods-thumb"><a href="<?php echo urlMall('goods','index',array('goods_id'=>$order['goods_id']));?>" target="_blank"><img src="<?php echo thumb($order,60);?>" onMouseOver="toolTip('<img src=<?php echo thumb($order,240);?>>')" onMouseOut="toolTip()"/></a></div></td>
      <td class="tl"><dl class="goods-name">
          <dt><a target="_blank" href="<?php echo urlMall('goods','index',array('goods_id'=>$order['goods_id']));?>"><?php echo $order['goods_name']; ?></a><a target="_blank" class="blue" href="<?php echo urlMall('vr_snapshot', 'index', array('order_id' => $order['order_id']));?>">[交易快照]</a></dt>
      ...
      <td><p><?php echo ncPriceFormat($order['goods_price']); ?></p>
        ...
        <span class="sale-type">团购活动</span>
        <?php } ?></td>
      ...
      <td class="bdl"><p class="feiwast-order-amount"><?php echo $order['order_amount']; ?></p>
      ...
      <td class="bdl bdr"><p><?php echo $order['state_desc']; ?> </p>
        ...
        <p><a href="index.php?app=store_vr_order&feiwa=show_order&order_id=<?php echo $order['order_id'];?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p></td>
      <td class="bdl bdr"><!-- 取消订单 -->
        
        <?php if($order['if_cancel']) { ?>
        <p><a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit mt5" feiwa_type="dialog" uri="index.php?app=store_vr_order&feiwa=change_state&state_type=order_cancel&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-circle"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
    </tr>
    <?php } } else { ?>
    <tr>
