...
<div class="eject_con">
  ...
  <form id="message" target="_parent" method="post" action="index.php?app=store_consult&feiwa=reply_save&consult_id=<?php echo $output['consult']['consult_id'];?>">
    <input type="hidden" name="type" value="<?php echo $output['consult']['consult_reply']==""?"reply":"edit"?>"/>
    <dl>
      <dt><?php echo $lang['store_consult_list_consult_content'].$lang['feiwa_colon'];?></dt>
      <dd><p style="line-height: 18px;"><?php echo nl2br($output['consult']['consult_content']);?></p></dd>
    </dl>
    <dl>
      <dt><i class="required">*</i><?php echo $lang['store_consult_list_reply_consult'].$lang['feiwa_colon'];?></dt>
      <dd>
    ...
    <div class="bottom">
        <label class="submit-border"><input type="submit" class="submit" value="<?php echo $lang['feiwa_ok'];?>" /></label>
    </div>
  </form>
</div>
<script>
