...
<form method="post" action="index.php?app=taobao_import&feiwa=index" enctype="multipart/form-data" id="goods_form">
  <div class="feiwast-form-goods"  <?php if($output['step'] != '1'){?> style="display:none"<?php }?>>
    <dl>
    ...
    <dl>
      ...
      <dd id="gcategory"> <span nctype="gc1">
        ...
        <select nctype="gc" data-param="{deep:1}">
          <option><?php echo $lang['feiwa_please_choose'];?></option>
    ...
    <dl>
      ...
      <dd><span class="new_add"><a href="javascript:void(0)" id="add_sgcategory" class="feiwast-btn"><?php echo $lang['store_goods_index_new_class'];?></a> </span>
...
function getClassSpec($this) {
    ...
    $.getJSON('index.php?app=store_spec&feiwa=ajax_class&id=' + id + '&deep=' + deep, function(data){
...
function nextClass(data, deep) {
    $('span[nctype="gc' + deep + '"]').html('').append('<select data-param="{deep:' + deep + '}"></select>')
        .find('select').change(function(){
            getClassSpec($(this));
        }).append('<option><?php echo $lang['feiwa_please_choose'];?></option>');
