...
.sticky .tabmenu { padding: 0;  position: relative; }
        ...
        <span class="fr mr5"> <a href="<?php echo urlMall('store_deliver', 'waybill_print', array('order_id' => $output['order_info']['order_id']));?>" class="ncbtn-mini" target="_blank" title="打印运单"/><i class="icon-print"></i>打印运单</a></span>
<div class="wrap">
  <div class="step-title"><em><?php echo $lang['store_deliver_first_step'];?></em><?php echo $lang['store_deliver_confirm_trade'];?></div>
  <form name="deliver_form" method="POST" id="deliver_form" action="index.php?app=store_deliver&feiwa=send&order_id=<?php echo $_GET['order_id'];?>" onsubmit="ajaxpost('deliver_form', '', '', 'onerror');return false;">
    ...
    <table class="feiwast-default-table order deliver">
      <tbody>
        ...
        <tr>
          <th colspan="20"><a href="index.php?app=store_order&feiwa=order_print&order_id=<?php echo $output['order_info']['order_id'];?>" target="_blank" class="fr" title="<?php echo $lang['store_show_order_printorder'];?>"/><i class="print-order"></i></a><span class="fr mr30"></span><span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['feiwa_colon'];?><?php echo $output['order_info']['order_sn']; ?></span><span class="ml20"><?php echo $lang['store_order_add_time'].$lang['feiwa_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$output['order_info']['add_time']); ?></em></span>
        </tr>
        <?php foreach($output['order_info']['extend_order_goods'] as $k => $goods_info) { ?>
        <tr>
          <td class="bdl w10"></td>
          <td class="w50"><div class="pic-thumb"><a href="<?php echo urlMall('goods','index',array('goods_id'=>$goods_info['goods_id']));?>" target="_blank"><img src="<?php echo cthumb($goods_info['goods_image'],'60',$$output['order_info']['store_id']); ?>" /></a></div></td>
          <td class="tl"><dl class="goods-name">
              <dt><a target="_blank" href="<?php echo urlMall('goods','index',array('goods_id'=>$goods_info['goods_id']));?>"><?php echo $goods_info['goods_name']; ?></a></dt>
          ...
          <td class="bdl bdr order-info w500" rowspan="<?php echo count($output['order_info']['extend_order_goods']);?>"><dl>
              <dt><?php echo $lang['store_deliver_shipping_amount'].$lang['feiwa_colon'];?></dt>
              <dd>
            ...
            <dl>
              <dt><?php echo $lang['store_deliver_forget'].$lang['feiwa_colon'];?></dt>
              <dd>
        ...
        <?php }?>
        <tr>
          <td colspan="20" class="tl bdl bdr" style="padding:8px" id="address"><strong class="fl"><?php echo $lang['store_deliver_buyer_adress'].$lang['feiwa_colon'];?></strong><span id="buyer_address_span"><?php echo $output['order_info']['extend_order_common']['reciver_name'];?>&nbsp;<?php echo $output['order_info']['extend_order_common']['reciver_info']['phone'];?>&nbsp;<?php echo $output['order_info']['extend_order_common']['reciver_info']['address'];?></span><?php echo $output['order_info']['extend_order_common']['reciver_info']['dlyp'] ? '[自提服务站]' : '';?>
          <a href="javascript:void(0)" feiwa_type="dialog" dialog_title="<?php echo $lang['store_deliver_buyer_adress'];?>" dialog_id="edit_buyer_address" uri="index.php?app=store_deliver&feiwa=buyer_address_edit&order_id=<?php echo $output['order_info']['order_id'];?>" dialog_width="550" class="ncbtn-mini fr"><i class="icon-edit"></i><?php echo $lang['feiwa_edit'];?></a></td>
        </tr>
        <?php } else { ?>
        <tr>
    ...
    <div class="deliver-sell-info"><strong class="fl"><?php echo $lang['store_deliver_my_daddress'].$lang['feiwa_colon'];?></strong>
      <a href="javascript:void(0);" onclick="ajax_form('modfiy_daddress', '<?php echo $lang['store_deliver_select_daddress'];?>', 'index.php?app=store_deliver&feiwa=send_address_select&order_id=<?php echo $output['order_info']['order_id'];?>', 640,0);" class="ncbtn-mini fr"><i class="icon-edit"></i><?php echo $lang['feiwa_edit'];?></a> <span id="seller_address_span">
    ...
    <table class="feiwast-default-table order" id="texpress1">
      <tbody>
        <tr>
          ...
          <td class="bdr w90 tc"><?php echo $lang['feiwa_common_button_operate'];?></td>
        </tr>
        ...
        <tr>
          ...
          <td class="bdl"><input name="shipping_code" type="text" class="text w200 tip-r" title="<?php echo $lang['store_deliver_shipping_code_tips'];?>" maxlength="20" feiwa_type='eb' feiwa_value="<?php echo $v;?>" /></td>
          <td class="bdl gray" feiwa_value="<?php echo $v;?>"></td>
          <td class="bdl bdr tc"><a feiwa_type='eb' feiwa_value="<?php echo $v;?>" href="javascript:void(0);" class="ncbtn"><?php echo $lang['feiwa_common_button_confirm'];?></a></td>
        </tr>
        <?php }?>
        <?php }?>
      </tbody>
    </table>
    <table class="feiwast-default-table order" id="texpress2" style="display:none">
      <tbody>
        <tr>
          <td colspan="2"></td>
        </tr>
        <tr>
          ...
          <td class="bdr tl w400">&emsp;<a feiwa_type='eb' feiwa_value="e1000" href="javascript:void(0);" class="ncbtn"><?php echo $lang['feiwa_common_button_confirm'];?></a></td>
        </tr>
        <tr>
          <td colspan="2"></td>
        </tr>
...
$(function(){
	...
	$('a[feiwa_type="eb"]').on('click',function(){
		if ($('input[feiwa_value="'+$(this).attr('feiwa_value')+'"]').val() == ''){
			showDialog('<?php echo $lang['store_deliver_shipping_code_pl'];?>', 'error','','','','','','','','',2);return false;
		}
		$('input[feiwa_type="eb"]').attr('disabled',true);
		$('input[feiwa_value="'+$(this).attr('feiwa_value')+'"]').attr('disabled',false);
		$('#shipping_express_id').val($(this).attr('feiwa_value'));
		$('#deliver_form').submit();
	});
    ...
    <?php if ($output['order_info']['shipping_code'] != ''){?>
    	$('input[feiwa_value="<?php echo $output['order_info']['extend_order_common']['shipping_express_id'];?>"]').val('<?php echo $output['order_info']['shipping_code'];?>');
    	$('td[feiwa_value="<?php echo $output['order_info']['extend_order_common']['shipping_express_id'];?>"]').html('<?php echo $output['order_info']['extend_order_common']['deliver_explain'];?>');
    <?php } ?>

    $('#my_address_add').click(function(){
		ajax_form('my_address_add', '<?php echo $lang['store_deliver_add_daddress'];?>' , 'index.php?app=store_deliver&feiwa=send_address_edit', 550,0);
    });
});
</script>
