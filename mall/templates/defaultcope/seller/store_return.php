...
<form method="get" action="index.php">
  <input type="hidden" name="app" value="store_return" />
...
<table class="feiwast-default-table">
  <thead>
    <tr>
      ...
      <th><?php echo $lang['feiwa_handle'];?></th>
    </tr>
  </thead>
  ...
  <tbody>
    ...
    <tr class="bd-line" >
        <td></td>
        <td class="w50"><div class="pic-thumb"><a href="<?php echo urlMall('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank">
		...
		<dl class="goods-name"><dt><a href="<?php echo urlMall('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank"><?php echo $val['goods_name']; ?></a></dt>
        <dd><?php echo $lang['refund_order_ordersn'].$lang['feiwa_colon'];?><a href="index.php?app=store_order&feiwa=show_order&order_id=<?php echo $val['order_id']; ?>" target="_blank"><?php echo $val['order_sn'];?></a></dd>
        <dd><?php echo $lang['return_order_returnsn'].$lang['feiwa_colon'];?><?php echo $val['refund_sn']; ?></dd></dl></td>
      ...
      <td class="nscs-table-handle">
        ...
        <span><a href="index.php?app=store_return&feiwa=edit&return_id=<?php echo $val['refund_id']; ?>" class="btn-bluejeans"><i class="icon-edit"></i><p>处理</p></a></span>
        <?php } else { ?>
       <span> <a href="index.php?app=store_return&feiwa=view&return_id=<?php echo $val['refund_id']; ?>" class="btn-bittersweet"><i class="icon-eye-open"></i><p><?php echo $lang['feiwa_view'];?></p></a></span>
        <?php } ?>
        <?php if ($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 2) { ?>
        <span><a href="javascript:void(0)" class="btn-mint" feiwa_type="dialog" dialog_title="<?php echo $lang['feiwa_edit'];?>" dialog_id="return_order" dialog_width="480" uri="index.php?app=store_return&feiwa=receive&return_id=<?php echo $val['refund_id']; ?>"><i class="icon-check-sign"></i><p><?php echo '收货';?></p></a></span>
        <?php } ?>
        </td>
    </tr>
    <?php } ?>
