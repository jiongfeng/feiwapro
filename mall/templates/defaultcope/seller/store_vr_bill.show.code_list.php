<?php defined('ByFeiWa') or exit('Access Invalid!');?>
  <form method="get" id="formSearch">
    <table class="search-form">
      <input type="hidden" id='app' name='app' value='store_vr_bill' />
      <input type="hidden" id='feiwa' name='feiwa' value='show_bill' />
      ...
      <tr>
        ...
        <td class="tc w200">
        <label class="submit-border"><input type="button" id="ncsubmit" class="submit" value="<?php echo $lang['feiwa_search'];?>" /></label>
...
<table class="feiwast-default-table">
    ...
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td>
       	<a target="_blank" href="index.php?app=store_vr_order&feiwa=show_order&order_id=<?php echo $code_info['order_id'];?>"><?php echo $lang['feiwa_view'];?></a>
        </td>
      </tr>
      <?php } ?>
      <?php } else { ?>
...
$(function(){
    $('#ncexport').click(function(){
    	$('input[name="feiwa"]').val('export_order');
    	$('#formSearch').submit();
    });
    $('#ncsubmit').click(function(){
    	$('input[name="feiwa"]').val('show_bill');
    	$('#formSearch').submit();
    });
});
</script>