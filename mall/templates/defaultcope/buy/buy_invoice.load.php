...
<ul>
  ...
  <li class="inv_item <?php echo $k == 0 ? 'ncc-selected-item' : null; ?>">
    <input content="<?php echo $val['content'];?>" id="inv_<?php echo $val['inv_id']; ?>" feiwa_type="inv" type="radio" name="inv" value="<?php echo $val['inv_id']; ?>" <?php echo $k == 0 ? 'checked' : null; ?>/>
    <label for="inv_<?php echo $val['inv_id']; ?>">&nbsp;&nbsp;<?php echo $val['content']; ?></label>
    &emsp;&emsp;&emsp;<a href="javascript:void(0);" onclick="delInv(<?php echo $val['inv_id']?>);" class="del">[ <?php echo $lang['feiwa_delete'];?> ]</a> </li>
  <?php } ?>
  <li class="inv_item">
    <?php if (count($output['inv_list']) < 10) {?>
    <input value="0" feiwa_type="inv" id="add_inv" type="radio" name="inv">
  ...
  <div id="add_inv_box" style="display:none">
    <form method="POST" id="inv_form" action="index.php">
      ...
      <input type="hidden" value="add_inv" name="feiwa">
      <input type="hidden" name="form_submit" value="ok"/>
      <div class="ncc-form-default">
        <dl>
          <dt>发票类型<?php echo $lang['feiwa_colon'];?></dt>
          <dd>
            <label>
      ...
      <div id="invoice_panel" class="ncc-form-default">
        <dl>
          <dt>发票抬头<?php echo $lang['feiwa_colon'];?></dt>
          <dd>
        ...
        <dl>
          <dt>发票内容<?php echo $lang['feiwa_colon'];?></dt>
          <dd>
      ...
      <div id="vat_invoice_panel" class="ncc-form-default" style="display:none">
        ...
        <dl>
          <dt><i class="required">*</i>发票内容<?php echo $lang['feiwa_colon'];?></dt>
          <dd>明细</dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票人姓名<?php echo $lang['feiwa_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_rec_name" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票人手机号<?php echo $lang['feiwa_colon'];?></dt>
          <dd>
            <input type="text" class="text w200" maxlength="50" name="inv_rec_mobphone" value="">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>收票人省份<?php echo $lang['feiwa_colon'];?></dt>
          <dd><input type="hidden" id="vregion" name="vregion">
          </dd>
        </dl>
        <dl>
          <dt><i class="required">*</i>送票地址<?php echo $lang['feiwa_colon'];?></dt>
          <dd>
...
function delInv(id){
    $('#invoice_list').load(SITEURL+'/index.php?app=buy&feiwa=load_inv&vat_hash<?php echo $_GET['vat_hash'];?>&del_id='+id);
}
$(function(){
	$('#vregion').feiwa_region();
    ...
    //使用新的发票信息
    $('input[feiwa_type="inv"]').on('click',function(){
        if ($(this).val() == '0') {
