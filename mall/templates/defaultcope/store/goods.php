...
<script type="text/javascript" src="<?php echo MALL_RESOURCE_SITE_URL;?>/js/mz-packed.js" charset="utf-8"></script>
<script type="text/javascript" src="<?php echo MALL_RESOURCE_SITE_URL;?>/js/jquery.SuperSlide.2.1.1.js" charset="utf-8"></script>
<div class="wrapper pr">
  <input type="hidden" id="lockcompare" value="unlock" />
  <div class="feiwa-s-main<?php if ($output['store_info']['is_own_mall']) echo ' ownmall'; ?>"> 
  	<!-- 店铺信息 -->
  	
  	<!-- 店铺信息 -->
   <!-- 焦点图 -->
	    <div id="feiwa-item-pics" class="feiwa-item-pics">
...
<!--        <dl>
        ...
        <dl>
          ...
          <dd class="price">
            ...
            <?php } else {?>
            <span><i class="c"><?php echo $lang['currency'];?></i><?php echo ncPriceFormat($output['goods']['goods_price']);?></span>
            <?php }?>
          </dd>
        </dl>
        <dl class="rate">
          <!-- S 描述相符评分 -->
          <dd><span><a href="#ncGoodsTraded">已售出<i class="num"><?php echo $output['goods']['goods_salenum']; ?></i></a></span><span class="b">评价<i class="num"><?php echo $output['goods_evaluate_info']['star_average'];?></i><span class="raty" data-score="<?php echo $output['goods_evaluate_info']['star_average'];?>"></span></span><span><a href="#ncGoodsRate">累积评价<i class="num"><?php echo $output['goods']['evaluation_count']; ?></i></a></span></dd>
          <!-- E 描述相符评分 -->
        </dl>
        ...
        <div class="limitBox">
                              距结束<b class="infoTit1 time-remain" count_down="<?php echo $output['goods']['xs_time']-TIMESTAMP;?>"><i></i><em time_id="d">0</em>天<em time_id="h">0</em>:<em time_id="m">0</em>:<em time_id="s">0</em></b>
                            </div>      <?php }?>
      </div>
        <!-- E 商品发布价格 -->
        <?php } else {?>
        ...
        <dl>
        	 <dt>预定价格</dt>
          <dd class="price">
            <span><i class="a"><?php echo $lang['currency'];?></i><i class="b"><?php echo $lang['currency'].ncPriceFormat((floatval($output['goods']['book_down_payment']) + floatval($output['goods']['book_final_payment'])));?></i></span><em><del><?php echo $lang['currency'].$output['goods']['goods_price'];?></del></em>
        </dl>
       </div>
        <div class="feiwa-ss-book-down">
        ...
        <dl>
          <dt>预定人数<?php echo $lang['feiwa_colon'];?></dt>
          <dd><strong><?php echo $output['goods']['book_buyers']; ?></strong> 人</dd>
        </dl>
        <dl>
          <dt>预定结束<?php echo $lang['feiwa_colon'];?></dt>
      ...
      <!-- S 促销 -->
      <?php if (isset($output['goods']['promotion_type']) || $output['goods']['have_gift'] == '1' || !empty($output['goods']['jjg_explain']) || !empty($output['mansong_info'])) {?>
      <div class="feiwa-ss-sale">
      <?php if (isset($output['goods']['promotion_type']) || !empty($output['goods']['jjg_explain']) || !empty($output['mansong_info'])) {?>
        <dl>
          ...
          <dd class="promotion-info">
            ...
            <div class="feiwa-ss-mansong"> <span class="sale-name">满即送</span> <span class="sale-rule">
              <?php $rule = $output['mansong_info']['rules'][0]; echo $lang['feiwa_man'];?>
              <em><?php echo $lang['currency'].ncPriceFormat($rule['price']);?></em>
              <?php if(!empty($rule['discount'])) { ?>
              ，<?php echo $lang['feiwa_reduce'];?><em><?php echo $lang['currency'].ncPriceFormat($rule['discount']);?></em>
              <?php } ?>
              <?php if(!empty($rule['goods_id'])) { ?>
              ，<?php echo $lang['feiwa_gift'];?><a href="<?php echo $rule['goods_url'];?>" title="<?php echo $rule['mansong_goods_name'];?>" target="_blank">赠品</a>
              <?php } ?>
              ...
              <div class="sale-rule-content" style="display: none;" nctype="rule-content">
                ...
                <div class="content">
                  <div class="mjs-tit"><?php echo $output['mansong_info']['mansong_name'];?>
                    <time>( <?php echo $lang['feiwa_promotion_time'];?><?php echo $lang['feiwa_colon'];?><?php echo date('Y-m-d',$output['mansong_info']['start_time']).'--'.date('Y-m-d',$output['mansong_info']['end_time']);?> )</time>
                  </div>
                  <ul class="mjs-info">
                    <?php foreach($output['mansong_info']['rules'] as $rule) { ?>
                    <li> <span class="sale-rule"><?php echo $lang['feiwa_man'];?><em><?php echo $lang['currency'].ncPriceFormat($rule['price']);?></em>
                      <?php if(!empty($rule['discount'])) { ?>
                      ， <?php echo $lang['feiwa_reduce'];?><em><?php echo $lang['currency'].ncPriceFormat($rule['discount']);?></em>
                      <?php } ?>
                      <?php if(!empty($rule['goods_id'])) { ?>
                      ， <?php echo $lang['feiwa_gift'];?> <a href="<?php echo $rule['goods_url'];?>" title="<?php echo $rule['mansong_goods_name'];?>" target="_blank" class="gift"> <img src="<?php echo cthumb($rule['goods_image'], 60);?>" alt="<?php echo $rule['mansong_goods_name'];?>"> </a>&nbsp;。
                      <?php } ?>
                      </span> </li>
                    <?php } ?>
                  </ul>
                ...
                <div class="bottom"><a href="<?php echo urlMall('show_store', 'mansong_goods', array('store_id' => $output['store_info']['store_id']));?>" class="url" target="_blank">查看更多店铺“满即送”活动商品</a></div>
              </div>
            </div>
            <?php } ?>
            <!--E 满就送 --> 
            ...
            <div class="feiwa-ss-jjg"> <span class="sale-name">加价购</span> <span class="sale-rule"><?php echo $output['goods']['jjg_explain']; ?></span> <span nctype="show-rule" class="sale-rule-more"><a href="javascript:void(0);">共<strong><?php echo count($output['goods']['jjg_info']['levels']);?></strong>项，展开查看<i></i></a></span>
              <div class="sale-rule-content" style="display: none;" nctype="rule-content">
...
$shownLevelSkus[$sku] = true; ?>
                        <li title="<?php echo $g['name']; ?>"><a mxf="sqde" target="_blank" href="<?php echo urlMall('goods', 'index', array('goods_id' => $sku)); ?>"> <img alt="" src="<?php echo cthumb($g['goods_image'], 60); ?>"/>
                ...
                <div class="bottom"><a href="<?php echo urlMall('show_store', 'cou_goods', array('store_id' => $output['store_info']['store_id'], 'cou_id' => $output['goods']['jjg_info']['info']['id']));?>" class="url" target="_blank">查看更多店铺“加价购”活动商品</a></div>
              </div>
            </div>
            <?php } ?>
            <!-- E 加价购 --> 
        ...
        <?php }?>
        <!-- S 赠品 -->
        <?php if ($output['goods']['have_gift'] == '1') {?>
        <hr/>
        <dl>
          <dt>赠&#12288;&#12288;品：</dt>
          <dd class="goods-gift" id="ncsGoodsGift"> <span>数量有限，赠完为止。</span>
            ...
            <ul>
              ...
              <li>
                ...
                <a href="<?php echo urlMall('goods', 'index', array('goods_id' => $val['gift_goodsid']));?>" class="goods-gift-name" target="_blank"><?php echo $val['gift_goodsname']?></a><em>x<?php echo $val['gift_amount'];?></em> </li>
              <?php }?>
            </ul>
            <?php }?>
          </dd>
      ...
      <div class="feiwa-ss-logistics"><!-- S 物流与运费新布局展示 ByFeiWaFeiWa -->
        <?php if($output['goods']['goods_state'] == 1 && $output['goods']['goods_verify'] == 1 && $output['goods']['is_virtual'] == 0){ ?>
        <dl id="feiwa-ss-freight" class="feiwa-ss-freight">
          <dt>配&nbsp;&nbsp;送&nbsp;&nbsp;至：</dt>
          <dd class="feiwa-ss-freight_box">
            <div id="feiwa-ss-freight-selector" class="feiwa-ss-freight-select">
              <div class="text">
                ...
                <b>∨</b> </div>
              <div class="content">
                <div id="feiwa-ss-stock" class="feiwa-ss-stock" data-widget="tabs">
                  <div class="mt">
                    <ul class="tab">
              ...
              <a href="javascript:;" class="close" onclick="$('#feiwa-ss-freight-selector').removeClass('hover')">关闭</a> </div>
            <div id="feiwa-ss-freight-prompt"> <strong><?php echo $output['goods']['goods_storage'] > 0 ? '有货' : '无货'?></strong>
      ...
      <div class="feiwa-ss-key"> 
        ...
        <dl nctype="nc-spec">
          <dt><?php echo $val;?><?php echo $lang['feiwa_colon'];?></dt>
          <dd>
      ...
      <div class="feiwa-ss-buy">
        ...
        <div class="feiwa-ss-point" style="display: none;"><i></i> 
          <!-- S 库存 --> 
          <span>您选择的商品库存<strong nctype="goods_stock"><?php echo $output['goods']['goods_storage']; ?></strong><?php echo $lang['feiwa_jian'];?></span> 
          <!-- E 库存 --> 
        ...
        <!-- S 提示已选规格及库存不足无法购买 -->
        <?php if ($output['goods']['goods_state'] == 0 || $output['goods']['goods_storage'] <= 0) {?>
        <div nctype="goods_prompt" class="feiwa-ss-point"><i></i> <span>您选择的商品<strong>库存不足</strong>；请选择店内其它商品或申请<a href="javascript:void(0);" nctype="arrival_notice" class="arrival" title="到货通知">到货通知</a>提示。</span> </div>
        <?php }?>
        <!-- E 提示已选规格及库存不足无法购买 --> 
        <!-- S 购买按钮 -->
        <div class="feiwa-ss-btn">
          <?php if ($output['goods']['cart'] == true) {?>
          <!-- 加入购物车--> 
          <a href="javascript:void(0);" nctype="addcart_submit" class="addcart <?php if ($output['goods']['goods_state'] == 0 || $output['goods']['goods_storage'] <= 0) {?>no-addcart<?php }?>" title="<?php echo $lang['goods_index_add_to_cart'];?>"><i class="icon-mallping-cart"></i><?php echo $lang['goods_index_add_to_cart'];?></a>
          <?php } ?>
          ...
          <?php }?>
          <!-- S 加入购物车弹出提示框 -->
          <div class="feiwa-ss-cart-popup">
            <dl>
              <dt><?php echo $lang['goods_index_cart_success'];?><a title="<?php echo $lang['goods_index_close'];?>" onClick="$('.feiwa-ss-cart-popup').css({'display':'none'});">X</a></dt>
              <dd><?php echo $lang['goods_index_cart_have'];?> <strong id="bold_num"></strong> <?php echo $lang['goods_index_number_of_goods'];?> <?php echo $lang['goods_index_total_price'];?><?php echo $lang['feiwa_colon'];?><em id="bold_mly" class="saleP"></em></dd>
              <dd class="btns"><a href="javascript:void(0);" class="ncbtn-mini ncbtn-mint" onClick="location.href='<?php echo MALL_SITE_URL.DS?>index.php?app=cart'"><?php echo $lang['goods_index_view_cart'];?></a> <a href="javascript:void(0);" class="ncbtn-mini" value="" onClick="$('.feiwa-ss-cart-popup').css({'display':'none'});"><?php echo $lang['goods_index_continue_mallping'];?></a></dd>
            </dl>
          </div>
          <!-- E 加入购物车弹出提示框 --> 
        </div>
      ...
      <div class="feiwa-ss-buy">
        <div class="feiwa-ss-saleout">
          <dl>
            ...
            <dd><?php echo $lang['goods_index_is_no_show_message_two_1'];?>&nbsp;<a href="<?php echo urlMall('show_store', 'index', array('store_id'=>$output['goods']['store_id']), $output['store_info']['store_domain']);?>" class="ncbtn-mini"><?php echo $lang['goods_index_is_no_show_message_two_2'];?></a>&nbsp;<?php echo $lang['goods_index_is_no_show_message_two_3'];?> </dd>
          </dl>
        </div>
      </div>
      <?php }?>
      <!-- E 购买数量及库存 --> 
      <!-- S消费者保障服务 -->
      <?php if($output['goods']["contractlist"] || $output['goods']["invite_rate"]>0){?>
      <div class="feiwa-ss-cti">
        <hr/>
        <dl>
          <dt>服务承诺：</dt>
          <dd>
		  <?php if($output['goods']["invite_rate"]>0){ ?>
          		<span><img src="/wap/images/mc_05.png"/>分销商品</span>
          	<?php } ?>
    ...
    <!-- E 商品图片及收藏分享 -->
    <div class="feiwa-ss-handle">
    	<span>编号：<?php echo $output['goods']['goods_id'];?></span>
      <!-- S 分享 --> 
      <a href="javascript:void(0);" class="share" feiwa_type="sharegoods" data-param='{"gid":"<?php echo $output['goods']['goods_id'];?>"}'><i></i><?php echo $lang['goods_index_snsshare_goods'];?><span>(<em feiwa_type="sharecount_<?php echo $output['goods']['goods_id'];?>"><?php echo intval($output['goods']['sharenum'])>0?intval($output['goods']['sharenum']):0;?></em>)</span></a> 
      <!-- S 收藏 --> 
      <a href="javascript:collect_goods('<?php echo $output['goods']['goods_id']; ?>','count','goods_collect');" class="favorite"><i></i>收藏<span>(<em nctype="goods_collect"><?php echo $output['goods']['goods_collect']?></em>)</span></a> 
      <!-- S 对比 --> 
      <a href="javascript:void(0);" class="compare" feiwa_type="compare_<?php echo $output['goods']['goods_id'];?>" data-param='{"gid":"<?php echo $output['goods']['goods_id'];?>"}'><i></i>对比</a><!-- S 举报 -->
      <?php if($output['inform_switch']) { ?>
      <a href="<?php if ($_SESSION['is_login']) {?>index.php?app=member_inform&feiwa=inform_submit&goods_id=<?php echo $output['goods']['goods_id'];?><?php } else {?>javascript:login_dialog();<?php }?>" title="<?php echo $lang['goods_index_goods_inform'];?>" class="inform"><i></i><?php echo $lang['goods_index_goods_inform'];?></a>
      <?php } ?>
      <!-- End --> </div>
    
    <!--S 店铺信息-->
    <div style="position: absolute; z-index: 2; top: -1px; right: -1px;">
      <?php include template('store/look_or_look');?>
  </div> <!--E 店铺信息 --> 
  <div class="clear"></div>
</div>
<!-- S 优惠套装 -->
<div class="feiwa-ss-promotion" id="nc-bundling" style="display:none;"></div>
<!-- E 优惠套装 -->
<div id="content" class="feiwa-ss-goods-layout expanded" >
  <div class="feiwa-ss-goods-main" id="main-nav-holder">
    <div class="tabbar pngFix" id="main-nav">
      <div class="feiwa-ss-goods-title-nav">
        <ul id="categorymenu">
          ...
          <li class="saoma"><i>扫一扫手机下单</i><img src="<?php echo goodsQRCode($output['goods']);?>"  title="用商城手机客户端扫描二维码直达商品详情内容"></li>
        </ul>
        <div class="switch-bar"><a href="javascript:void(0)" id="fold">&nbsp;</a></div>
      </div>
    </div>
    <div class="feiwa-ss-intro">
      <div class="content bd" id="ncGoodsIntro">
        ...
        <ul class="nc-goods-sort">
          ...
          <?php if(isset($output['goods']['brand_name'])){echo '<li>'.$lang['goods_index_brand'].$lang['feiwa_colon'].$output['goods']['brand_name'].'</li>';}?>
          <?php if(is_array($output['goods']['goods_attr']) && !empty($output['goods']['goods_attr'])){?>
          <?php foreach ($output['goods']['goods_attr'] as $val){ $val= array_values($val);echo '<li>'.$val[0].$lang['feiwa_colon'].$val[1].'</li>'; }?>
          <?php }?>
          <?php if (is_array($output['goods']['goods_custom'])) {
            foreach ($output['goods']['goods_custom'] as $val) {
                if ($val['value']!= '') {
                    echo '<li>'.$val['name'].$lang['feiwa_colon'].$val['value'].'</li>';
                }
            }
            }?>
        </ul>
        <?php }?>
        <div class="feiwa-ss-goods-info-content">
    ...
    <?php if ($output['goods']['is_virtual'] == 1) {?>
    <!-- S 店铺地址地图 -->
    <div class="feiwa-ss-mall-map">
      <div class="feiwa-ss-goods-title-bar hd">
    ...
    <!-- E 店铺地址地图 -->
    <?php }?>
    <div class="feiwa-ss-comment">
      <div class="feiwa-ss-goods-title-bar hd">
        <h4><a href="javascript:void(0);"><?php echo $lang['goods_index_evaluation'];?></a></h4>
      </div>
      <div class="feiwa-ss-goods-info-content bd" id="ncGoodsRate">
        <div class="top">
          <div class="rate">
          ...
          <div class="btns"><span>您可对已购商品进行评价</span>
            <p><a href="<?php if ($output['goods']['is_virtual']) { echo urlMall('member_vr_order', 'index');} else { echo urlMall('member_order', 'index');}?>" class="ncbtn ncbtn-grapefruit" target="_blank"><i class="icon-comment-alt"></i>评价商品</a></p>
          </div>
        </div>
        <div class="feiwa-ss-goods-title-nav">
          <ul id="comment_tab">
        ...
        <!-- 商品评价内容部分 -->
        <div id="goodseval" class="feiwa-ss-commend-main"></div>
      </div>
    </div>
    <div class="ncg-salelog">
      ...
      <div class="feiwa-ss-goods-info-content bd" id="ncGoodsTraded">
        <div class="top">
        ...
        <!-- 成交记录内容部分 -->
        <div id="salelog_demo" class="feiwa-ss-loading"> </div>
      </div>
    </div>
    <div class="feiwa-ss-consult">
      <div class="feiwa-ss-goods-title-bar hd">
        <h4><a href="javascript:void(0);"><?php echo $lang['goods_index_goods_consult'];?></a></h4>
      </div>
      <div class="feiwa-ss-goods-info-content bd" id="ncGuestbook"> 
        <!-- 咨询留言内容部分 -->
        <div id="consulting_demo" class="feiwa-ss-loading"> </div>
      </div>
    </div>
<!-- feiwa-->
<!--youce-->
  ...
  <div class="feiwa-ss-sidebar">
    ...
    <div class="feiwa-ss-sidebar-container feiwa-ss-top-bar">
      ...
      <div class="content">
        ...
        <p><a href="<?php echo MALL_SITE_URL;?>/index.php?app=member_goodsbrowse&feiwa=list" class="nch-sidebar-all-viewed">全部浏览历史</a></p> </div>
    </div>
    <?php } ?>
  </div>
</div>
...
$(function(){
    ...
    $('a[nctype="arrival_notice"]').click(function(){
        ...
        login_dialog();
        <?php }else{?>
        ajax_form('arrival_notice', '到货通知','<?php echo urlMall('goods', 'arrival_notice', array('goods_id' => $output['goods']['goods_id']));?>', 350);
        <?php }?>
    });
    <?php }?>
    //浮动导航  waypoints.js
    ...
    $(".feiwa-look-slider .picScroll-top").slide({
	   titCell:".hd ul",
	   mainCell:".bd ul",
	   autoPage:true,
	   effect:"top",
...
$(function(){
    ...
    $('#transport_pannel>a').click(function(){
	    ...
	    var url = 'index.php?app=goods&feiwa=calc&rand='+Math.random();
	    $('#transport_price').css('display','none');
	    $('#loading_price').css('display','');
	    $.getJSON(url, {'id':id,'tid':tpl_id}, function(data){
	    	if (data == null) return false;
	        if(data != 'undefined') {$('#feiwa_kd').html('运费<?php echo $lang['feiwa_colon'];?><em>' + data + '</em><?php echo $lang['goods_index_yuan'];?>');}else{'<?php echo $lang['goods_index_trans_for_seller'];?>';}
	        $('#transport_price').css('display','');
	    	$('#loading_price').css('display','none');
	        $('#ncrecive').html($(_self).html());
	    });
    });
    $("#nc-bundling").load('index.php?app=goods&feiwa=get_bundling&goods_id=<?php echo $output['goods']['goods_id'];?>', function(){
        if($(this).html() != '') {
            $(this).show();
        }
    });
    <?php if ($output['goods']['is_virtual'] == 1) {?>
    $("#ncStoreMap").load('index.php?app=show_map&feiwa=index&w=420&h=400&store_id=<?php echo $output['goods']['store_id'];?>', function(){
        if($(this).html() != '') {
            $(this).show();
        }
    });
    <?php } ?>
    $("#salelog_demo").load('index.php?app=goods&feiwa=salelog&goods_id=<?php echo $output['goods']['goods_id'];?>&store_id=<?php echo $output['goods']['store_id'];?>&vr=<?php echo $output['goods']['is_virtual'];?>', function(){
        // Membership card
        $(this).find('[nctype="mcard"]').membershipCard({type:'mall'});
    });
	$("#consulting_demo").load('index.php?app=goods&feiwa=consulting&goods_id=<?php echo $output['goods']['goods_id'];?>&store_id=<?php echo $output['goods']['store_id'];?>', function(){
		// Membership card
		$(this).find('[nctype="mcard"]').membershipCard({type:'mall'});
	});
...
/** goods.php **/
	// 商品内容部分折叠收起侧边栏控制
	$('#fold').click(function(){
  		$('.feiwa-ss-goods-layout').toggleClass('expanded');
	});
	// 商品内容介绍Tab样式切换控制
	...
	//商品排行Tab切换
	$(".feiwa-ss-top-tab > li > a").mouseover(function(e) {
		if (e.target == this) {
			var tabs = $(this).parent().parent().children("li");
			var panels = $(this).parent().parent().parent().children(".feiwa-ss-top-panel");
	...
	//信用评价动态评分打分人次Tab切换
	$(".feiwa-ss-rate-tab > li > a").mouseover(function(e) {
		if (e.target == this) {
			var tabs = $(this).parent().parent().children("li");
			var panels = $(this).parent().parent().parent().children(".feiwa-ss-rate-panel");
			var index = $.inArray(this, $(this).parent().parent().find("a"));
			if (panels.eq(index)[0]) {
				tabs.removeClass("current ").eq(index).addClass("current ");
				panels.addClass("hide").eq(index).removeClass("hide");
...
//触及显示缩略图
    ...
    function load_goodseval(type) {
        var url = '<?php echo urlMall('goods', 'comments', array('goods_id' => $output['goods']['goods_id']));?>';
        url += '&type=' + type;
    ...
    //记录浏览历史
	$.get("index.php?app=goods&feiwa=addbrowse",{gid:<?php echo $output['goods']['goods_id'];?>});
	//初始化对比按钮
	initCompare();

    $('[nctype="show_chain"]').click(function(){
        _goods_id = $(this).attr('data-goodsid');
        ajax_form('show_chain', '查看门店', 'index.php?app=goods&feiwa=show_chain&goods_id=' + _goods_id, 640);
    });
    ...
    $('.feiwa-ss-buy').bind({
        mouseover:function(){$(".feiwa-ss-point").show();},
        mouseout:function(){$(".feiwa-ss-point").hide();}
    });
...
function addcart_callback(data){
    ...
    $('.feiwa-ss-cart-popup').fadeIn('fast');
}

<?php if($output['goods']['goods_state'] == 1 && $output['goods']['goods_verify'] == 1 && $output['goods']['is_virtual'] == 0){ ?>
var $cur_area_list,$cur_tab,next_tab_id = 0,cur_select_area = [],calc_area_id = '',calced_area = [],cur_select_area_ids = [];
$(document).ready(function(){
	$("#feiwa-ss-freight-selector").hover(function() {
		//如果店铺没有设置默认显示区域，马上异步请求
		<?php if (!$output['store_info']['deliver_region']) { ?>
		if (typeof feiwa_a === "undefined") {
	 		$.getJSON(SITEURL + "/index.php?app=index&feiwa=json_area&callback=?", function(data) {
	 			feiwa_a = data;
	 			$cur_tab = $('#feiwa-ss-stock').find('li[data-index="0"]');
	 			_loadArea(0);
	 		});
		}
		<?php } ?>
	...
	$('ul[class="area-list"]').on('click','a',function(){
		$('#feiwa-ss-freight-selector').unbind('mouseleave');
		...
		$cur_tab = $('#feiwa-ss-stock').find('li[data-index="'+tab_id+'"]');
		...
		if (tab_id < 2) {
			...
			cur_select_area.push($(this).html());
			cur_select_area_ids.push(area_id);
			$('#feiwa-ss-freight-selector > div[class="text"] > div').html(cur_select_area.join(''));
			$('#feiwa-ss-freight-selector').removeClass("hover");
			_calc();
		}
		$('#feiwa-ss-stock').find('li[data-widget="tab-item"]').on('click','a',function(){
			var tab_id = parseInt($(this).parent().attr('data-index'));
			if (tab_id < 2) {
				$(this).parent().nextAll().remove();
				$(this).addClass('hover');
				$('#feiwa-ss-stock').find('div[data-widget="tab-content"]').each(function(){
	...
	function _loadArea(area_id){
		if (feiwa_a[area_id] && feiwa_a[area_id].length > 0) {
			$('#feiwa-ss-stock').find('div[data-widget="tab-content"]').each(function(){
			...
			$('#feiwa-ss-freight-selector > div[class="text"] > div').html(cur_select_area);
			$('#feiwa-ss-freight-selector').removeClass("hover");
			_calc();
		}
	}
	...
	function _calc() {
		...
		if (_args != '') {
			...
			if (typeof calced_area[calc_area_id] == 'undefined') {
				//需要请求配送区域设置
				$.getJSON(SITEURL + "/index.php?app=goods&feiwa=calc&" + _args + "&myf=<?php echo $output['store_info']['store_free_price']?>&callback=?", function(data){
					allow_buy = data.total ? true : false;
					calced_area[calc_area_id] = data.total;
					if (data.total === false) {
						$('#feiwa-ss-freight-prompt > strong').html('无货').next().remove();
						$('a[nctype="buynow_submit"]').addClass('no-buynow');
						$('a[nctype="addcart_submit"]').addClass('no-buynow');
					} else {
						$('#feiwa-ss-freight-prompt > strong').html('有货 ').next().remove();
						$('#feiwa-ss-freight-prompt > strong').after('<span>' + data.total + '</span>');
				...
				if (calced_area[calc_area_id] === false) {
					...
					$('#feiwa-ss-freight-prompt > strong').html('有货 ').next().remove();
					$('#feiwa-ss-freight-prompt > strong').after('<span>' + calced_area[calc_area_id] + '</span>');
					$('a[nctype="buynow_submit"]').removeClass('no-buynow');
					$('a[nctype="addcart_submit"]').removeClass('no-buynow');
				}
			}
