...
<div class="goods-pic"><span class="thumb size120"><i></i><img src="<?php echo cthumb($output['sharegoods_info']['snsgoods_goodsimage'], 240,$output['sharegoods_info']['snsgoods_storeid']);?>"/></span></div>
  <ul id="goods_images">
    <?php for($i=0;$i<5;$i++){?>
    <li feiwa_type="handle_pic" data-param="{'apid':'<?php echo isset($output['pic_list'][$i])?$output['pic_list'][$i]['ap_id']:0;?>'}" >
      ...
      <div class="bg" feiwa_type="handler" style="display:none;"><span class="delete" title="<?php echo $lang['feiwa_delete'];?>" nctype="drop_image"><?php echo $lang['feiwa_delete'];?></span></div>
      <div class="upload-btn"><a href="javascript:void(0);"><span style="width: 66px; height: 28px; position: absolute; left: 0; top: 0; z-index: 999; cursor:pointer; ">
        ...
        <input id="submit_button" style="display:none" type="button" value="<?php echo $lang['feiwa_submit'];?>" onClick="submit_form($(this))">
        </a></div>
    </li>
    <?php }?>
  </ul>
...
$(function(){
	...
	$('span[nctype="drop_image"]').click(function(){
		...
		$.getJSON('index.php?app=member_snshome&feiwa=del_sharepic', {apid:data_str.apid}, function(data){
			if(data.type == 'false'){
				...
				p_li.find('*[feiwa_type="handler"]').hide().css('z-index', '0');
				img_handle();
			}
		});
	});
...
function ajaxFileUpload(id){
	...
	$.ajaxFileUpload
	({
		url:SITEURL + '/index.php?app=member_snshome&feiwa=image_upload',
		secureuri:false,
		fileElementId:id,
		dataType: 'json',
