...
<div class="wrap">
  ...
  <form method="get" action="index.php">
    <table class="ncm-search-table">
      <input type="hidden" name="app" value="member_return" />
  ...
  <table class="ncm-default-table order">
    ...
    <tbody>
      ...
      <tr>
        <th colspan="20"> <span class="fl ml10"><?php echo $lang['return_order_returnsn'].$lang['feiwa_colon'];?><?php echo $val['refund_sn']; ?></span><span><?php echo $lang['refund_buyer_add_time'].$lang['feiwa_colon'];?><?php echo date("Y-m-d H:i:s",$val['add_time']);?></span>
          <!-- store_name -->
          <span> <a href="<?php echo urlMall('show_store','index',array('store_id'=> $val['store_id']), $output['store_list'][$val['store_id']]['store_domain']);?>" title="<?php echo $order_info['store_name'];?>"><?php echo $val['store_name']; ?></a></span>
          <!-- QQ -->
      ...
      <tr>
        <td class="bdl"></td>
        <td class="w50"><div class="pic-thumb"><a href="<?php echo urlMall('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank"><img src="<?php echo thumb($val,60);?>" onMouseOver="toolTip('<img src=<?php echo thumb($val,60);?>>')" onMouseOut="toolTip()"/></a></div></td>
        <td class="tl"><dl class="goods-name">
            <dt><a href="<?php echo urlMall('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank"><?php echo $val['goods_name']; ?></a></dt>
            <dd><?php echo $lang['refund_order_ordersn'].$lang['feiwa_colon'];?><a href="index.php?app=member_order&feiwa=show_order&order_id=<?php echo $val['order_id']; ?>" target="_blank"><?php echo $val['order_sn'];?></a></dd>
          </dl></td>
        ...
        <td class="bdr"><?php if($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 1) { ?>
          <p><a href="index.php?app=member_return&feiwa=ship&return_id=<?php echo $val['refund_id']; ?>" class="ncbtn">退货</a></p>
          <?php } else { ?>
          <a href="index.php?app=member_return&feiwa=view&return_id=<?php echo $val['refund_id']; ?>" class="ncbtn">查看</a>
          <?php } ?>
          <?php if($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 3) { ?>
          <p><a href="javascript:void(0)" class="ncbtn ncbtn-bittersweet" feiwa_type="dialog" dialog_title="延迟时间" dialog_id="return_delay" dialog_width="480"
        	    uri="index.php?app=member_return&feiwa=delay&return_id=<?php echo $val['refund_id']; ?>"> 延时 </a></p>
          <?php } ?></td>
      </tr>
      <?php } ?>
      <?php } else { ?>
