  ...
  <table class="ncm-default-table order">
    ...
    <tbody>
      <tr>
        ...
        <td><a href="<?php echo urlMall('goods', 'index', array('goods_id' => $val['goods_id']));?>" target="_blank"><?php echo $val['goods_name'];?></a></td>
        <td><?php echo date('Y-m-d H:i:s', $val['an_addtime']);?></td>
        <td class="ncm-table-handle"><span><a class="btn-grapefruit" onclick="ajax_get_confirm('您确定要取消预约/到货通知吗？', 'index.php?app=member_appoint&feiwa=del_appoint&id=<?php echo $val['an_id'];?>');" href="javascript:void(0);"><i class="icon-trash"></i><p>删&nbsp;除</p></a></span></td>
      </tr>
    </tbody>
    <?php }?>
    <?php } else { ?>
