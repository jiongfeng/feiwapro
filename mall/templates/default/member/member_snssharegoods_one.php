...
<div class="dialog-share">
  <form method="post" action="index.php?app=member_snsindex&feiwa=sharegoods" id="sharegoods_form">
    ...
    <div class="share-set">
      <div class="share-widget"><?php echo $lang['sharebind_alsoshareto'];?>
        <ul class="s-app">
          ...
          <li feiwa_type="appitem_<?php echo $k;?>" title="<?php echo $v['name'];?>" class="<?php echo $v['isbind']?'checked':'disable';?>"> <i class="i-<?php echo $k;?>" feiwa_type="bindbtn" data-param='{"apikey":"<?php echo $k;?>","apiname":"<?php echo $v['name'];?>"}' attr_isbind="<?php echo $v['isbind']?'1':'0';?>"></i>
      ...
      <div feiwa_type="formprivacydiv" class="share-privacy"><a href="javascript:void(0);" feiwa_type="formprivacybtn"><i class="icon-group"></i>可见范围</a>
        <div class="privacytab" feiwa_type="formprivacytab" style="display:none;"> <span class="arrow"></span>
          <ul>
            <li feiwa_type="formprivacyoption" data-param='{"v":"0","hiddenid":"gprivacy"}'><span class="selected"></span><?php echo $lang['sns_weiboprivacy_all'];?></li>
            <li feiwa_type="formprivacyoption" data-param='{"v":"1","hiddenid":"gprivacy"}'><span></span><?php echo $lang['sns_weiboprivacy_friend'];?></li>
            <li feiwa_type="formprivacyoption" data-param='{"v":"2","hiddenid":"gprivacy"}'><span></span><?php echo $lang['sns_weiboprivacy_self'];?></li>
          </ul>
        </div>
      </div>
    </div>
    <div class="share-content">
      ...
      <div class="share-goods">
        <div class="goods-thumb"><a target="_blank" href="<?php echo urlMall('goods','index',array('goods_id'=>$output['goods_info']['goods_id']));?>"> <img title="<?php echo $output['goods_info']['goods_name']; ?>" src="<?php echo thumb($output['goods_info'],240);?>" /></a></div>
        <dl class="goods-intro">
          <dt><a target="_blank" href="<?php echo urlMall('goods','index',array('goods_id'=>$output['goods_info']['goods_id']));?>"><?php echo $output['goods_info']['goods_name']; ?></a></dt>
    ...
    <!-- 验证码 -->
    <div id="sg_seccode" class="seccode"><?php echo $lang['feiwa_checkcode'].$lang['feiwa_colon']; ?>
    ...
    <!-- 防止点击Enter键提交 -->
    <div class="bottom">
      <input name="<?php echo $lang['feiwa_snsshare'];?>" type="button" class="button" value="<?php echo $lang['feiwa_snsshare'];?>" id="weibobtn_goods" />
...
<div class="bottom"><a href="javascript:void(0);" feiwa_type="finishbtn" data-param='{"apikey":"@apikey"}' class="mini-btn"><?php echo $lang['sharebind_list_finishbind'];?></a><span><?php echo $lang['sharebind_list_unfinishedbind'];?><a target="_blank" href="<?php echo MEMBER_SITE_URL;?>/api.php?app=sharebind&type=@apikey"><?php echo $lang['sharebind_list_againbind'];?></a></span>
</div>
</div>
</textarea>
	...
	$("[feiwa_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){//已经绑定
		    if($("#checkapp_"+data_str.apikey).val() == '1'){
			    $("[feiwa_type='appitem_"+data_str.apikey+"']").removeClass('checked');
		    	$("[feiwa_type='appitem_"+data_str.apikey+"']").addClass('disable');
            	$("#checkapp_"+data_str.apikey).val('0');
			}else{
				$("[feiwa_type='appitem_"+data_str.apikey+"']").removeClass('disable');
            	$("[feiwa_type='appitem_"+data_str.apikey+"']").addClass('checked');
		    ...
		    window.open(MEMBER_SITE_URL + '/api.php?app=sharebind&type='+data_str.apikey);
		}
	});
	$("[feiwa_type='finishbtn']").live('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
		//验证是否绑定成功
