...
<?php if ($output['order_info']) {?>
  ...
  <form id="changeform" method="post" action="index.php?app=store_deliver&feiwa=delay_receive&order_id=<?php echo $output['order_info']['order_id']; ?>">
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
      <dt><?php echo $lang['store_order_sn'].$lang['feiwa_colon'];?></dt>
      <dd><span class="num"><?php echo $output['order_info']['order_sn']; ?></span></dd>
    </dl>
    <dl>
      <dt><?php echo $lang['store_order_buyer_with'].$lang['feiwa_colon'];?></dt>
      <dd><?php echo $output['order_info']['buyer_name']; ?></dd>
    </dl>
    <dl>
      <dt><?php echo '最晚收货时间'.$lang['feiwa_colon'];?></dt>
      <dd><?php echo date('Y-m-d H:i:s',$output['order_info']['delay_time']);?><br/>如果超过该时间买家未点击收货，系统将自动更改为收货状态 </dd>
    </dl>
    <dl>
      <dt><?php echo '延迟'.$lang['feiwa_colon'];?></dt>
      <dd>
    ...
    <div class="bottom">
        <label class="submit-border"><input type="submit" class="submit" id="confirm_button" value="<?php echo $lang['feiwa_ok'];?>" /></label>
    </div>
  </form>
<?php } else { ?>
