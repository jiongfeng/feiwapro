...
<div class="feiwast-oredr-show">
  <div class="feiwast-order-info">
    <div class="feiwast-order-details">
      <div class="title"><?php echo $lang['store_show_order_info'];?></div>
      <div class="content">
        <dl>
          <dt><?php echo $lang['store_show_order_receiver'].$lang['feiwa_colon'];?></dt>
        ...
        <dl>
          <dt><?php echo $lang['store_order_pay_method'].$lang['feiwa_colon'];?></dt>
        ...
        <dl>
          <dt><?php echo $lang['store_show_order_buyer_message'].$lang['feiwa_colon'];?></dt>
          <dd><?php echo $output['order_info']['extend_order_common']['order_message']; ?></dd>
        </dl>
        <dl class="line">
          <dt><?php echo $lang['store_order_order_sn'].$lang['feiwa_colon'];?></dt>
    ...
    <div class="feiwast-order-condition">
      <dl>
    ...
    <div class="feiwast-order-condition">
      ...
      <ul>
        ...
        <li>2. 可以在<a href="index.php?app=store_evaluate&feiwa=list" class="ncbtn-mini">评价管理</a>查看详细内容。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="feiwast-order-condition">
  ...
  <div id="order-step" class="feiwast-order-step">
  ...
  <div class="feiwast-order-contnet">
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td class="tl"><dl class="goods-name">
              <dt><a target="_blank" href="<?php echo $goods['goods_url']; ?>"><?php echo $goods['goods_name']; ?></a><a target="_blank" href="<?php echo urlMall('snapshot', 'index', array('rec_id' => $goods['rec_id']));?>"  class="blue ml5">[交易快照]</a></dt>
              ...
              <dd>
                ...
                退款单号：<a target="_blank" href="index.php?app=store_refund&feiwa=view&refund_id=<?php echo $output['refund_all']['refund_id'];?>"><?php echo $output['refund_all']['refund_sn'];?></a>
                <?php }else if($goods['extend_refund']['refund_type'] == 1) {?>
                退款单号：<a target="_blank" href="index.php?app=store_refund&feiwa=view&refund_id=<?php echo $goods['extend_refund']['refund_id'];?>"><?php echo $goods['extend_refund']['refund_sn'];?></a></dd>
              <?php }else if($goods['extend_refund']['refund_type'] == 2) {?>
              退货单号：<a target="_blank" href="index.php?app=store_return&feiwa=view&return_id=<?php echo $goods['extend_refund']['refund_id'];?>"><?php echo $goods['extend_refund']['refund_sn'];?></a>
              </dd>
              <?php } ?>
            </dl></td>
          ...
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php echo $output['order_info']['state_desc']; ?>
            ...
            <p><a href="javascript:void(0)" class="ncbtn" uri="index.php?app=store_order&feiwa=change_state&state_type=modify_price&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" feiwa_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $output['order_info']['order_id']; ?>_action_adjust_fee" />修改运费</a></p>
            <?php }?>
            
            <!-- 取消订单 -->
            
            <?php if ($output['order_info']['if_cancel']) { ?>
            <p><a href="javascript:void(0)" style="color:#F30; text-decoration:underline;" feiwa_type="dialog" uri="index.php?app=store_order&feiwa=change_state&state_type=order_cancel&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $output['order_info']['order_id']; ?>_action_cancel" /><?php echo $lang['store_order_cancel_order'];?></a></p>
            <?php } ?>
            
            <!-- 发货 -->
            
            <?php if ($output['order_info']['if_store_send']) { ?>
            <p><a class="ncbtn" href="index.php?app=store_deliver&feiwa=send&order_id=<?php echo $output['order_info']['order_id']; ?>"/><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></p>
            <?php } ?></td>
          <?php } ?>
          <!-- E 合并TD --> 
        </tr>
        ...
        <tr class="bd-line">
          <td>&nbsp;</td>
          <td colspan="6" class="tl"><div class="feiwast-goods-gift">赠品：
              <ul>
      ...
      <tfoot>
        ...
        <tr>
          <td colspan="20"><dl class="freight">
            ...
            <dl class="sum">
              <dt><?php echo $lang['store_order_sum'].$lang['feiwa_colon'];?></dt>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?app=seller_center&feiwa=get_express&e_code=<?php echo $output['order_info']['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['shipping_code']?>&t=<?php echo random(7);?>',function(data){
    		if(data){
    			data = var_send+data;
