...
<?php } else {?>
  <strong>套餐过期时间<?php echo $lang['feiwa_colon'];?></strong> <strong style=" color:#F00;"><?php echo date('Y-m-d H:i:s',$output['combo_quota']['cq_endtime']);?></strong>
<?php }?>
  <ul>
...
<table class="feiwast-default-table">
  ...
  <tbody nctype="choose_goods_list">
    ...
    <tr class="bd-line">
      <td></td>
      <td><div class="pic-thumb"><a href="<?php echo urlMall('goods', 'index', array('goods_id' => $val['goods_id']));?>" target="black"><img src="<?php echo thumb($val, 60);?>"/></a></div></td>
      <td class="tl">
        <dl class="goods-name">
	...
	<td class="nscs-table-handle">
        <span>
          <a class="btn-bluejeans" href="<?php echo urlMall('store_promotion_combo', 'choosed_goods', array('gid' => $val['goods_id']));?>">
            <i class="icon-edit"></i>
            <p>编辑</p>
          </a>
        </span>
