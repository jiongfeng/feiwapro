...
<div class="alert alert-block mt10">
  <?php if(!empty($output['current_xianshi_quota'])) { ?>
  <strong>套餐过期时间<?php echo $lang['feiwa_colon'];?></strong><strong style="color:#F00;"><?php echo date('Y-m-d H:i:s', $output['current_xianshi_quota']['end_time']);?></strong>
  <?php } else { ?>
...
<form method="get">
  <table class="search-form">
    <input type="hidden" name="app" value="store_promotion_xianshi" />
    <input type="hidden" name="feiwa" value="xianshi_list" />
    <tr>
      <td>&nbsp;</td>
      <th>状态</th>
...
<table class="feiwast-default-table">
  <thead>
    <tr>
      ...
      <th class="w80">状态</th>
      <th class="w160"><?php echo $lang['feiwa_handle'];?></th>
    </tr>
  </thead>
  ...
  <tbody id="xianshi_list">
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle tr"><?php if($val['editable']) { ?>
        <span> <a href="index.php?app=store_promotion_xianshi&feiwa=xianshi_edit&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-bluejeans"> <i class="icon-edit"></i>
        <p><?php echo $lang['feiwa_edit'];?></p>
        </a> </span>
        <?php } ?>
        <span> <a href="index.php?app=store_promotion_xianshi&feiwa=xianshi_manage&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-mint"> <i class="icon-cog"></i>
        <p><?php echo $lang['feiwa_manage'];?></p>
        ...
        <p><?php echo $lang['feiwa_delete'];?></p>
        </a> </span></td>
    </tr>
    <?php }?>
    <?php }else{?>
