...
<div class="base-layout">
  <div class="mainbox">
    ...
    <form id="circle_form" action="<?php echo CIRCLE_SITE_URL;?>/index.php?app=manage&c_id=<?php echo $output['c_id'];?>" method="post" class="base-form-style">
      <input type="hidden" value="ok" name="form_submit">
      <dl>
        <dt><?php echo $lang['circle_desc'].$lang['feiwa_colon'];?></dt>
        <dd>
      ...
      <dl>
        <dt><?php echo $lang['circle_notice'].$lang['feiwa_colon'];?></dt>
        <dd>
      ...
      <dl>
        <dt><?php echo $lang['circle_logo'].$lang['feiwa_colon'];?></dt>
        <dd>
...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url:'index.php?app=cut&feiwa=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_CIRCLE;?>/group',
				secureuri:false,
				fileElementId:'c_img',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['feiwa_cut'];?>','index.php?app=cut&feiwa=pic_cut&filename=<?php echo ATTACH_CIRCLE;?>/group/<?php echo intval($_GET['c_id']);?>.jpg&x=120&y=120&resize=1&url='+data.url,680);
					}else{
						alert(data.msg);
					}
    ...
    $('#circle_form').validate({
    	...
    	submitHandler:function(form){
    		ajaxpost('circle_form', '<?php echo CIRCLE_SITE_URL;?>/index.php?app=manage&c_id=<?php echo $output['c_id'];?>', '', 'onerror');
    	},
        rules : {
        	c_desc : {
            	maxlength : 255
