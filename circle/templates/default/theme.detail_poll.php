...
<div class="theme-detail-poll-content">
  <form method="post" id="poll_form" <?php if(!$output['vote_end'] && !$output['partake']){?>action="<?php echo CIRCLE_SITE_URL;?>/index.php?app=theme&feiwa=save_votepoll&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>"<?php }?>>
    ...
    <table width="100%" border=0 cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td><?php if($val['pollop_votername'] != ''){?>
          <span><?php echo recentlyTwoVoters($val['pollop_votername']);?>&nbsp;<?php echo $lang['feiwa_etc'];?></span>
          <?php }?>
    ...
    <div class="bottom"><a href="javascript:void(0);" nctype="poll_submit" class="btn" ><?php echo $lang['feiwa_submit'];?></a><span nctype="poll-warning" class="warning"></span> </div>
    <?php }?>
  </form>
</div>
<script>
...
$(function(){
	...
	$('a[nctype="poll_submit"]').click(function(){
		if(_ISLOGIN){
			...
			ajaxpost('poll_form', CIRCLE_SITE_URL+'/index.php?app=theme&feiwa=save_votepoll&c_id='+c_id+'&t_id='+t_id, '', 'onerror');
		}else{
			login_dialog();		
		}
	});
