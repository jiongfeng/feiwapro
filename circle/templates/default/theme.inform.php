...
<div class="eject_con inform-reason">
  ...
  <form id="inform_form" action="<?php echo CIRCLE_SITE_URL;?>/index.php?app=theme_inform&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id']; if(isset($_GET['r_id'])){echo '&r_id='.$_GET['r_id'];}?>" method="post" onsubmit="ajaxpost('inform_form', '<?php echo CIRCLE_SITE_URL;?>/index.php?app=theme_inform&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id']; if(isset($_GET['r_id'])){echo '&r_id='.$_GET['r_id'];}?>', '', 'onerror');">
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
      <dt><?php echo $lang['circle_inform_reason'];?></dt>
      <dd>
        <h5>
          <p><?php echo $lang['circle_inform_choosable_reason'].$lang['feiwa_colon'];?></p>
    ...
    <div class="bottom">
      <a class="submit-btn" nctype="inform_submit" href="Javascript: void(0)"><?php echo $lang['feiwa_submit'];?></a>
      <a class="cancel-btn" nctype="inform_cancel" href="Javascript: void(0)"><?php echo $lang['feiwa_cancel'];?></a>
    </div>
  </form>
</div>
<script>
