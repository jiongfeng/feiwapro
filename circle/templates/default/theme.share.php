...
<div class="circle-share">
  <form method="post" action="<?php echo CIRCLE_SITE_URL;?>/index.php?app=theme_share&c_id=<?php echo $output['c_id'];?>&t_id=<?php echo $output['t_id'];?>" id="share_form" class="feededitor">
    ...
    <div class="share-widget"> <span class="title"><?php echo $lang['sharebind_alsoshareto'];?></span> <span class="s-app">
      ...
      <label feiwa_type="appitem_<?php echo $k;?>" title="<?php echo $v['name'];?>" class="<?php echo $v['isbind']?'checked':'disable';?>"> <i class="i-<?php echo $k;?>" feiwa_type="bindbtn" data-param='{"apikey":"<?php echo $k;?>","apiname":"<?php echo $v['name'];?>"}' attr_isbind="<?php echo $v['isbind']?'1':'0';?>"></i>
    ...
    <div class="handle">
      ...
      <input name="<?php echo $lang['feiwa_snsshare'];?>" type="button" class="button" value="<?php echo $lang['feiwa_snsshare'];?>" id="share_button" />
    </div>
...
<dl class="bottom"><dt style="width:25%">&nbsp;</dt>
        <dd style="width:75%"><a href="javascript:void(0);" feiwa_type="finishbtn" data-param='{"apikey":"@apikey"}' class="ncu-btn2 mr10"><?php echo $lang['sharebind_list_finishbind'];?></a><span><?php echo $lang['sharebind_list_unfinishedbind'];?><a target="_blank" href="<?php echo MEMBER_SITE_URL;?>/api.php?app=sharebind&type=@apikey" class="ml5"><?php echo $lang['sharebind_list_againbind'];?></a></span></dd>
      </dl>
</div>
</textarea>
...
$(function(){
	...
	$("[feiwa_type='finishbtn']").live('click',function(){
	    ...
	    eval( "data_str = "+data_str);
		// check binding
		var url = 'index.php?app=share&feiwa=checkbind';
