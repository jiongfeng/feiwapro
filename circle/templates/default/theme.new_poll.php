...
<div class="group warp-all">
  ...
  <div class="theme-editor">
    <form method="post" id="theme_form" action="<?php echo CIRCLE_SITE_URL;?>/index.php?app=theme&feiwa=save_theme&sp=1&c_id=<?php echo $output['c_id'];?>">
      <input type="hidden" name="form_submit" value="ok" />
      <div class="quick-thread">
        <div class="base-tab-menu">
          <ul class="base-tabs-nav">
            <li><a href="index.php?app=theme&feiwa=new_theme&c_id=<?php echo $output['c_id'];?>"><?php echo $lang['circle_new_theme'];?></a></li>
            <li class="selected"><a href="javascript:void(0);"><?php echo $lang['circle_new_poll'];?></a></li>
          </ul>
        </div>
        <div class="quick-thread-box">
          <div class="title">
            <label class="mr10"><span class="t"><?php echo $lang['circle_type'].$lang['feiwa_colon'];?></span><span class="i">
              <select name="thtype" class="select">
                <option value="0"><?php echo $lang['feiwa_default'];?></option>
            ...
            <label><span class="t"><?php echo $lang['feiwa_title'].$lang['feiwa_colon'];?></span><span class="i">
              <input name="name" type="text" class="text" />
              </span></label>
          </div>
          <div class="poll-options">
            <div class="top"> <span>
              <h4><?php echo $lang['circle_poll_options'].$lang['feiwa_colon'];?></h4>
              <?php echo $lang['circle_poll_options_max'];?> </span><span class="input-text">
              <h4><?php echo $lang['circle_poll_days'].$lang['feiwa_colon'];?></h4>
              <label>
                <input type="text" name="days" class="input-text" />
                <?php echo $lang['feiwa_day'];?></label>
              </span> <span id="poll_div_2" class="input-radio">
              <h4><?php echo $lang['circle_poll_patterns'].$lang['feiwa_colon'];?></h4>
              <label>
          ...
          <div class="bottom"> <a class="submit-btn" nctype="theme_submit" href="Javascript: void(0)"><?php echo $lang['feiwa_release_new_theme'];?></a> <a class="cancel-btn" nctype="theme_cancle" href="Javascript:history.go(-1);"><?php echo $lang['feiwa_cancel'];?></a>
            <div id="warning"></div>
          </div>
        </div>
      </div>
...
$(function(){
    ...
    $('#theme_form').validate({
    	...
    	submitHandler:function(form){
    		...
    		ajaxpost('theme_form', CIRCLE_SITE_URL+'/index.php?app=theme&feiwa=save_theme&c_id='+c_id, '', 'onerror');
    	},
    	focusInvalid : false,
        rules : {
        	name : {
        ...
        messages : {
        	name : {
                required : '<?php echo $lang['feiwa_name_not_null'];?>',
                minlength : '<?php echo $lang['feiwa_name_min_max_length'];?>',
            	maxlength : '<?php echo $lang['feiwa_name_min_max_length'];?>'
            },
            themecontent  : {
...
function addpolloption(o){
	...
	$("<p class=\"new-add\"><input type=\"text\" name=\"polloption[]\" value=\"\" class=\"option\" /><a href=\"javascript:void(0);\"><?php echo $lang['feiwa_delete'];?></a></p>").find('a').click(function(){
		$(this).parent().remove();
	}).end().insertBefore(o);
}
</script>
