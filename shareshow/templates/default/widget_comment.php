...
$(document).ready(function(){
     ...
     * 评论
     **/
    var url_comment_list = "index.php?app=comment&feiwa=comment_list&type=<?php echo $output['comment_type'];?>&comment_id=<?php echo $output['comment_id'];?>";
    var url_comment_save = "index.php?app=comment&feiwa=comment_save&type=<?php echo $output['comment_type'];?>";

    $("#comment_message").shareshow_publish({
        ...
        $.post(url_comment_save, $("#add_form").serialize(),
            function(data){
                if(data.result == 'true') {
                    ...
                    first_comment.find("a[feiwa_type=comment_drop]").attr('comment_id',data.comment_id);
                    first_comment.show('slow');
                    if($("#comment_list dl").length >= 5) {
                        $("#comment_list dl").last().remove();
                    }
                    $("#comment_count").shareshow_count({type:'+'});
                } else {
    ...
    //评论删除
    $("[feiwa_type=comment_drop]").live('click',function(){
        if(confirm('<?php echo $lang['feiwa_ensure_del'];?>')) {
            var item = $(this).parents("dl");
            $.getJSON("index.php?app=comment&feiwa=comment_drop&type=<?php echo $output['comment_type'];?>", { comment_id: $(this).attr("comment_id")}, function(json){
                if(json.result == "true") {
                    $("#comment_count").shareshow_count({type:'-'});
                    item.remove();
                } else {
...
<div id="widgetcommenttitle" class="widget-comment-title">
  <h3><?php echo $lang['shareshow_comment_title'];?></h3>
...
<form id="add_form">
  ...
  <div><a id="btn_show_smilies" href="javascript:void(0)" class="smile"><?php echo $lang['shareshow_comment_smiles'];?></a> 
    <!-- 站外分享 -->
    <?php require('widget_share.php');?>
  <input id="btn_publish_comment" class="input-button" type="button" value="<?php echo $lang['shareshow_text_publish'];?>" />
  </div>
</form>
</dd>
<div class="clear">&nbsp;</div>
...
<?php } else { ?>
<div class="login"> <?php echo $lang['no_login'];?> <a href="<?php echo urlLogin('login', 'index', array('ref_url' => getRefUrl()));?>"><?php echo $lang['feiwa_login'];?></a> </div>
<?php } ?>
...
<div id="_comment_list_template" style="display:none;">
  <dl>
    ...
    <dd> <a class="comment_member_info" href="" target="_blank"></a> <span></span>
      <p><em></em> <a feiwa_type="comment_drop" comment_id="" href="javascript:void(0)" class="del pngFix"><?php echo $lang['feiwa_delete'];?></a></p>
    </dd>
  </dl>
</div>
