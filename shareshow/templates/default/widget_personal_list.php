...
$(document).ready(function(){
    ...
    //喜欢
    $("[feiwa_type=shareshow_like]").shareshow_like({type:'personal'});
});
</script>
...
<div class="commend-goods-list">
  ...
  <ul id="pinterest">
    <li id="btn_personal_publish" class="item" style="display:none;">
      <div class="pinterest-button"><a id="btn_show_publish_div" href="javascript:void(0)" class="pngFix"><?php echo $lang['shareshow_personal_publish'];?></a></div>
    </li>
    ...
    <li class="item">
      ...
      <?php } else { ?>
      <!-- 个人主页删除按钮 -->
      <div class="del"><a feiwa_type="home_drop" type="personal" item_id="<?php echo $value['personal_id'];?>" href="javascript:void(0)"><?php echo $lang['feiwa_delete'];?></a></div>
      <?php } ?>
      <?php } ?>
      <div class="picture"> <a href="<?php echo SHARESHOW_SITE_URL;?>/index.php?app=personal&feiwa=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank">
        <?php $personal_image_array = getShareShowPersonalImageUrl($value,'list');?>
        <?php $size = getShareShowImageSize($personal_image_array[0], 238);?>
        <img class="lazy" height="<?php echo $size['height'];?>" width="<?php echo $size['width'];?>" src="<?php echo SHARESHOW_TEMPLATES_URL;?>/images/loading.gif" data-src="<?php echo $personal_image_array[0];?>" /> </a> </div>
      <div class="handle"> <span class="like-btn"><a feiwa_type="shareshow_like" like_id="<?php echo $value['personal_id'];?>" href="javascript:void(0)"><i class="pngFix"></i><span><?php echo $lang['shareshow_text_like'];?></span><em><?php echo $value['like_count']<=999?$value['like_count']:'999+';?></em></a></span> <span class="comment"><a href="<?php echo SHARESHOW_SITE_URL;?>/index.php?app=personal&feiwa=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank"><i title="<?php echo $lang['shareshow_text_comment'];?>" class="pngFix">&nbsp;</i><em><?php echo $value['comment_count']<=999?$value['comment_count']:'999+';?></em></a></span> </div>
      <dl class="commentary">
        ...
        <dd>
          ...
          <p> <a href="<?php echo SHARESHOW_SITE_URL;?>/index.php?app=personal&feiwa=detail&personal_id=<?php echo $value['personal_id'];?>" target="_blank"> <?php echo $value['commend_message'];?> </a> </p>
        </dd>
      </dl>
    </li>
    <?php } ?>
  ...
  <div class="no-content"><i class="personal pngFix">&nbsp;</i>
    <?php if($_GET['app'] == 'personal') { ?>
    <p><?php echo $lang['shareshow_personal_list_none'];?></p>
    <?php } else { ?>
    <?php if($_GET['feiwa'] == 'like_list') { ?>
    <?php if($output['owner_flag'] === TRUE){ ?>
    <p><?php echo $lang['shareshow_personal_like_list_none'];?></p>
    <?php } else { ?>
    <p><?php echo $lang['feiwa_quote1'];?><?php echo $output['member_info']['member_name'];?><?php echo $lang['feiwa_quote2'];?><?php echo $lang['shareshow_personal_like_list_none_owner'];?></p>
    <?php } ?>
    <?php } else { ?>
    <?php if($output['owner_flag'] === TRUE){ ?>
    <p><?php echo $lang['shareshow_personal_list_none_publish'];?></p>
    <a id="link_personal_publish" href="javascript:void(0)"><?php echo $lang['shareshow_personal_publish'];?></a></div>
  <?php } else { ?>
  <p><?php echo $lang['feiwa_quote1'];?><?php echo $output['member_info']['member_name'];?><?php echo $lang['feiwa_quote2'];?><?php echo $lang['shareshow_personal_list_none_publish_owner'];?></p>
  <?php } ?>
  <?php } ?>
  <?php } ?>
</div>
...
<script type="text/javascript" src="<?php echo SHARESHOW_RESOURCE_SITE_URL;?>/js/fileupload/jquery.iframe-transport.js" charset="utf-8"></script> 
<script type="text/javascript" src="<?php echo SHARESHOW_RESOURCE_SITE_URL;?>/js/fileupload/jquery.ui.widget.js" charset="utf-8"></script> 
<script type="text/javascript" src="<?php echo SHARESHOW_RESOURCE_SITE_URL;?>/js/fileupload/jquery.fileupload.js" charset="utf-8"></script> 
<!-- 发布个人秀 --> 
<script type="text/javascript">
$(document).ready(function(){

    $("#publish_div").shareshow_form({title:'<?php echo $lang['shareshow_personal_publish'];?>'});
    ...
    function show_publish_form() {
        $.getJSON("index.php?app=publish&feiwa=personal_limit", {}, function(json){
            if(json.result == "true") {
                $("#publish_div").shareshow_form_show({width:480});
            } else {
    ...
    //发布
    $("#commend_message").shareshow_publish({
        ...
        if($("#personal_image").val() != "") {
            ...
            show_personal_message("<?php echo $lang['shareshow_text_upload_image'];?>");
        }
    });
    ...
    $("#fileupload").fileupload({
        dataType: 'json',
            url: "<?php echo SHARESHOW_SITE_URL.DS.'index.php?app=publish&feiwa=personal_image_upload';?>",
            add: function(e,data) {
                ...
                if(result.status == "success") {
                    $("#_personal_image_list img").attr("src","<?php echo UPLOAD_SITE_URL.DS.ATTACH_SHARESHOW.DS.$_SESSION['member_id'].DS;?>"+result.file);
    ...
    //图片删除按钮
    $("[feiwa_type='btn_personal_image_delete']").live('click',function(){
        var image_name = $(this).attr("image_name");
        $.getJSON("<?php echo SHARESHOW_SITE_URL.DS.'index.php?app=publish&feiwa=personal_image_delete';?>", { image_name: image_name }, function(json){
        });
    ...
    $("#btn_personal_link_add").click(function() {
        ...
        if(link != '') {
            $.getJSON("<?php echo SHARESHOW_SITE_URL.DS.'index.php?app=publish&feiwa=personal_link_add';?>", { link: link}, function(data){
                if(data.result == "true") {
                    ...
                    link_item += '<span class="taobao-item-delete"><a feiwa_type="btn_personal_link_delete"><?php echo $lang['feiwa_delete'];?></a></span>';
    ...
    //删除购买链接
    $("[feiwa_type='btn_personal_link_delete']").live('click',function(){
...
<div id="_personal_image_list" style="display:none;">
  <div class="personal_image_content"><a feiwa_type="btn_personal_image_delete" image_name="" href="javascript:void(0);" class="personal_image_delete" style="display:none;"><?php echo $lang['feiwa_delete'];?></a> <span class=" thumb size100"><i></i><img src="" onload="javascript:DrawImage(this,100,100);" /></span></div>
</div>
</div>
<!-- 发布按钮 --> 
<!-- 弹出层开始 -->
<div id="publish_div" style="display: none;">
  <form action="<?php echo SHARESHOW_SITE_URL;?>/index.php?app=publish&feiwa=personal_save" method="post" id="add_form" class="publish-div">
    ...
    <div class="side-bar">
      ...
      <div id="personal_image_upload" class="personal-image-upload">
        <div class="upload-btn"><a href="javascript:void(0);"> <span style="width: 80px; height: 30px; position: absolute; left: 0; top: 0; z-index: 999; cursor:pointer; ">
          ...
          <input id="submit_button" style="display:none" type="button" value="<?php echo $lang['shareshow_text_upload_image'];?>" onClick="submit_form($(this))" />
          </a></div>
        <div id="personal_image_info" class="personal_image_info" style="display:none;" title="<?php echo $lang['shareshow_text_uploading'];?>">&nbsp;</div>
      </div>
    </div>
    <div class="express">
      <dl class="type">
        <dt><?php echo $lang['shareshow_text_class'];?></dt>
        <dd>
      ...
      <dl class="intro">
        <dt><?php echo $lang['shareshow_text_explain'];?></dt>
        <dd>
          <textarea cols="55" rows="3" name="commend_message" id="commend_message"></textarea>
        </dd>
      </dl>
      <dl class="url">
        <dt> <?php echo $lang['shareshow_text_buy_link'];?>
          <?php if(C('taobao_api_isuse')) { ?>
          <span><?php echo $lang['shareshow_personal_publish_explain_taobao'];?></span>
          <?php } else { ?>
          <span><?php echo $lang['shareshow_personal_publish_explain_store'];?></span>
          <?php } ?>
        </dt>
        <dd>
          ...
          <div id="div_personal_image_link" style="display:none;">
            ...
            <a id="btn_personal_link_add" href="javascript:void(0);" class="add-link"><?php echo $lang['feiwa_add'];?></a></div>
          <div id="personal_image_link"><a id="btn_personal_link" href="javascript:void(0);" class="add-btn"><?php echo $lang['shareshow_personal_add_link'];?></a> </div>
        </dd>
      </dl>
    </div>
    <div class="handle">
      <div id="personal_image_notice" class="personal_image_notice" style="display:none;"></div>
      <input id="btn_publish" type="button" value="<?php echo $lang['shareshow_text_commend'];?>" />
      <!-- 站外分享 -->
