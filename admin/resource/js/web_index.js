...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: 'index.php?app=web_config&feiwa=code_update',
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		case "recommend_list":
			...
			break;
		case "brand_list":
			$("#show_brand_list").load('index.php?app=web_config&feiwa='+id);//查询数据
...
function get_goods_class() {//查询子分类
	...
	if (gc_id>0) {
		$.get('index.php?app=web_config&feiwa=category_list&id='+gc_id, function(data) {
			$(data).each(function(){
				var obj = $(this);
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load('index.php?app=web_config&feiwa=recommend_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
	...
	for (var i = 1; i <= recommend_max; i++) {//防止数组下标重复
		if ($(".middle dl[recommend_id='"+i+"']").size()==0) {//编号不存在时添加
			...
			add_html = '<dl recommend_id="'+i+'"><dt><h4>商品推荐</h4>'+del_append+
    			'<a href="JavaScript:show_recommend_dialog('+i+');"><i class="fa fa-mallping-cart"></i>商品块</a><a href="JavaScript:show_recommend_pic_dialog('+i+');"><i class="icon-lightbulb"></i>广告块</a></dt>'+
...
function select_recommend_goods(goods_id) {//商品选择
	...
	var goods_jingle = goods.attr("goods_jingle");
	...
	text_append += '<a href="'+MALL_SITE_URL+'/index.php?app=goods&goods_id='+goods_id+'" target="_blank">';
	...
	text_append += '<input name="recommend_list['+id+'][goods_list]['+goods_id+'][goods_jingle]" value="'+goods_jingle+'" type="hidden">';
...
function get_recommend_brand() {//查询品牌
	...
	if (brand_name!='' || brand_initial!='') {
		$("#show_brand_list").load('index.php?app=web_config&feiwa=brand_list&'+$.param({'brand_name':brand_name,'brand_initial':brand_initial }));
	}
}
...
function update_brand() {//更新品牌
	...
	if (get_text=='1') {
		...
		$("img[select_brand_id]").each(function() {
		  ...
		  $(".feiwa-btbrand ul.brands").append('<li><span>'+text_append+'</span></li>');
		});
...
function select_slide_adv(pic_id) {//选中图片
	...
	//liuxuexin start
	if(pic_simg!=''){
		document.getElementById("image2").src='/data/upload/'+pic_simg;
	}else{
		document.getElementById("image2").src='/admin/templates/default/images/picture.gif';
	}	
	//liuxuexin end
...
function slide_adv(pic_id,pic_img,pic_simg) {//更新图片
    var obj = $("#upload_adv_form ul li[slide_adv_id='"+pic_id+"']");
	if (pic_img!='') {
        ...
        //var pic_simg = $("input[name='slide_pic[pic_simg]']").val(); by liuxuexin
	    obj.find("img").attr("title",pic_name);
	    obj.find("img").attr("src",UPLOAD_SITE_URL+'/'+pic_img);
	    //liuxuexin start
	    if(pic_simg!=''){
...
function get_goods_list(){//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_sale_goods_list").load('index.php?app=web_config&feiwa=goods_list&'+$.param({'id':gc_id,'goods_order':goods_order,'goods_name':goods_name }));
	}
}
...
function select_sale_goods(goods_id){//商品选择
	...
	text_append += '<span class="thumb size-72x72">';
	text_append += '<i></i>';
  	text_append += '<img select_goods_id="'+goods_id+'" title="'+goods_name+'" goods_name="'+goods_name+'" src="'+goods_pic+'" goods_price="'+goods_price+'"market_price="'+market_price+'" goods_jingle="'+goods_jingle+'"  onload="javascript:DrawImage(this,72,72);" />';
	text_append += '</span></div>';
	text_append += '<div class="goods-name">';
	text_append += '<a href="'+SITEURL+'/index.php?app=goods&goods_id='+goods_id+'" target="_blank">';
  	text_append += goods_name+'</a>';
	text_append += '</div>';
...
function update_sale(){//更新
    ...
    obj.find("img").each(function(){
    	...
    	text_append += '<input name="sale_list['+id+'][goods_list]['+goods_id+'][goods_jingle]" value="'+goods_jingle+'" type="hidden">';
