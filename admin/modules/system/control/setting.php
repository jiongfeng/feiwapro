...
class settingControl extends SystemControl{
    private $links = array(
        array('url'=>'app=setting&feiwa=base','lang'=>'web_set'),
        array('url'=>'app=setting&feiwa=dump','lang'=>'dis_dump'),
        array('url'=>'app=setting&feiwa=login','lang'=>'loginSettings'),
    );
    ...
    public function dumpFeiWa(){
        ...
        if (chksubmit()){
            ...
            $result = $model_setting->updateSetting($update_array);
            if ($result === true){
                $this->log(L('feiwa_edit,dis_dump'),1);
                showMessage(L('feiwa_common_save_succ'));
            }else {
                $this->log(L('feiwa_edit,dis_dump'),0);
                showMessage(L('feiwa_common_save_fail'));
            }
        }
        $list_setting = $model_setting->getListSetting();
