...
class chat_logControl extends SystemControl{
    ...
    public function get_xmlFeiwa() {
        ...
        if ($f_member['member_id'] > 0 && $t_member['member_id'] > 0) {//验证账号
			...
			$condition = array();
            if($f_member['member_id']!=""){
				$condition['f_id'] = intval($f_member['member_id']);
			}
            if($t_member['member_id']!=""){
				$condition['t_id'] = intval($t_member['member_id']);
			}
            $param = array('add_time', 'f_id', 't_id');
            $order = 'add_time desc';
        ...
        foreach ($log_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn green' href='javascript:void(0);' onclick='ajax_form(\"login\",\"消息内容\",\"". urlAdminMall('chat_log', 'chat_info', array('id' => $value['m_id'])) ."\",640)' class='url'><i class='fa fa-list-alt'></i>查看</a>";
     ...
     * 输出XML数据
     */
    public function get_msg_xmlFeiwa() {
        $model_chat = Model('web_chat');
        $log_list = array();
        if (!empty($_GET['msg'])) {
            $add_time_from = strtotime($_GET['add_time_from']);
        ...
        foreach ($log_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn green' href='index.php?app=chat_log&feiwa=chat_log&f_name=".$value['f_name']."&t_name=".$value['t_name']."&add_time_from=".date("Y-m-d",$value['add_time'])."&add_time_to=".date("Y-m-d",$value['add_time'])."' class='url'><i class='fa fa-list-alt'></i>详情</a>";
            $param['t_msg'] = $value['t_msg'];
            $param['add_time'] = date('Y-m-d H:i:s', $value['add_time']);
            $param['f_name'] = $value['f_name'];
            $param['f_id'] = $value['f_id'];
    ...
    public function chat_infoFeiwa() {
        $id = $_GET['id'];
