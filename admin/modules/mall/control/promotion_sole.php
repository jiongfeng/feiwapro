...
class promotion_soleControl extends SystemControl {
    public function __construct() {
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?app=promotion_sole&promotion_allow=1',
                    'msg'=>L('open'),
                ),
            );
     ...
     * 活动商品管理XML
     */
    public function goods_list_xmlFeiwa() {
        $condition = array();
        ...
        foreach ($goods_list as $val) {
            $o = '<a class="btn red confirm-del-on-click" href="javascript:;" data-href="' . urlAdminMall('promotion_sole', 'del_goods', array('goods_id' => $val['goods_id'])) . '"><i class="fa fa-trash"></i>删除</a>';

            $o .= '<a class="btn green" target="_blank" href="' . urlMall('goods', 'index', array('goods_id' => $val['goods_id'])) . '"><i class="fa fa-list-alt"></i>查看</a>';
            ...
            $i['store_name'] = '<a target="_blank" href="' . urlMall('show_store', 'index', array('store_id' => $val['store_id'])) . '">' . $val['store_name'] . '</a>';
...
EOB;
     ...
     * 套餐列表
     */
    public function sole_quota_listFeiwa() {
		Tpl::setDirquna('mall');/*www.feiwa.org*/
     ...
     * 套餐列表XML
     */
    public function sole_quota_list_xmlFeiwa() {
        $condition = array();
        if (strlen($q = trim($_REQUEST['query']))) {
            switch ($_REQUEST['qtype']) {
                case 'store_name':
        ...
        foreach ($list as $val) {
            ...
            $i['operation'] = '<a class="btn green" href="' . urlAdminMall('promotion_sole', 'goods_list', array('store_id' => $val['store_id'])) . '"><i class="fa fa-list-alt"></i>查看商品</a>';
     ...
     * 删除推荐商品
     */
    public function del_goodsFeiwa() {
        $goodsIds = array();
     ...
     * 设置
     */
    public function sole_settingFeiwa() {
        // 实例化模型
        $model_setting = Model('setting');
        ...
        if (chksubmit()){
            ...
            if($return){
                $this->log(L('feiwa_set').'手机专享套餐');
                showMessage(L('feiwa_common_op_succ'));
            }else{
