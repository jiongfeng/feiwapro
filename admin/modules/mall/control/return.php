...
class returnControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'app=return&feiwa=return_manage','text'=>'待处理'),
            array('url'=>'app=return&feiwa=return_all','text'=>'所有记录')
    );
    public function __construct(){
        parent::__construct();
        $model_refund = Model('refund_return');
        $model_refund->getRefundStateArray();
        if ($_GET['feiwa'] == 'index') $_GET['feiwa'] = 'return_manage';
        Tpl::output('top_link',$this->sublink($this->links,$_GET['feiwa']));
    }

    public function indexFeiwa() {
        $this->return_manageFeiwa();
    }

    /**
     * 待处理列表
     */
    public function return_manageFeiwa() {
     ...
     * 待处理列表
     */
    public function get_manage_xmlFeiwa() {
        $model_refund = Model('refund_return');
        $condition = array();
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?app=return&feiwa=edit&return_id={$return_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            ...
            $list['goods_name'] = "<a class='open' title='{$return_info['goods_name']}' href='". urlMall('goods', 'index', array('goods_id' => $return_info['goods_id'])) ."' target='blank'>{$return_info['goods_name']}</a>";
     ...
     * 所有记录
     */
    public function return_allFeiwa() {
		Tpl::setDirquna('mall');/*www.feiwa.org*/
     ...
     * 所有记录
     */
    public function get_all_xmlFeiwa() {
        $model_refund = Model('refund_return');
        $condition = array();
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"index.php?app=return&feiwa=view&return_id={$return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
    ...
    public function editFeiwa() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => urlMall('member_refund', 'view', array('return_id' => $return['refund_id'])),
                    'refund_sn' => $return['refund_sn']
                );
                QueueClient::push('sendMemberMsg', $param);

                showMessage(Language::get('feiwa_common_save_succ'),'index.php?app=return&feiwa=return_manage');
            } else {
                showMessage(Language::get('feiwa_common_save_fail'));
            }
        }
