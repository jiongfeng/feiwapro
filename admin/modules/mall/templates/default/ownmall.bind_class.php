...
<div class="page">
<div class="fixed-bar">
  <div class="item-title"><a class="back" href="index.php?app=ownmall&feiwa=list" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
    <div class="subject">
      <h3>自营店铺 - <?php echo $lang['feiwa_edit'];?>“<?php echo $output['store_info']['store_name'];?>”经营类目</h3>
      <h5>商城自营店铺相关设置与管理</h5>
    </div>
  </div>
</div>
...
<table class="flex-table">
  <thead>
    <tr>
      ...
      <th width="60" class="handle-s" align="center"><?php echo $lang['feiwa_handle'];?></th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="sort"><span feiwa_type="commis_rate" column_id="<?php echo $value['bid'];?>" title="<?php echo $lang['feiwa_editable'];?>" class="editable " style="vertical-align: middle; margin-right: 4px;"><?php echo $value['commis_rate'];?></span>% </td>
...
<div class="ncap-form-default" >
  ...
  <dl class="row">
    ...
    <dd class="opt">
      <form id="add_form" action="<?php echo urlAdminMall('ownmall', 'bind_class_add');?>" method="post">
...
$(function(){
    ...
    $('[nctype="btn_del_store_bind_class"]').on('click', function() {
        if(confirm('确认删除？删除后店铺对应分类商品将全部下架')) {
            ...
            $this = $(this);
            $.post('<?php echo urlAdminMall('ownmall', 'bind_class_del');?>', {bid: bid}, function(data) {
                 if(data.result) {
    ...
    // 修改分佣比例
    $('span[feiwa_type="commis_rate"]').inline_edit({app: 'ownmall',feiwa: 'bind_class_update'});
});
