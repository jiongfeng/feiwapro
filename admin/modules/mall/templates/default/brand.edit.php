...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?app=brand&feiwa=brand" title="返回品牌列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $lang['brand_index_brand'];?> - <?php echo $lang['feiwa_edit'];?>品牌“<?php echo $output['brand_array']['brand_name']?>”</h3>
  ...
  <form id="brand_form" method="post" name="form1" enctype="multipart/form-data">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        <dt class="tit">品牌大图背景</dt>
        <dd class="opt">
                    <div class="input-file-show"><span class="show"><a class="nyroModal" rel="gal" href="<?php echo $output['brand_array']['brand_bgpic']; ?>"> <i class="fa fa-picture-o" onMouseOver="toolTip('<img src=<?php echo $output['brand_array']['brand_bgpic']; ?>>')" onMouseOut="toolTip()"></i></a></span><span class="type-file-box">
            <input class="type-file-file" id="_bgpic" name="_pic" type="file" size="30" hidefocus="true" title="点击前方预览图可查看大图，点击按钮选择文件并提交表单后上传生效">
            <input type="text" name="brand_bgpic" id="brand_bgpic" class="type-file-text" />
            <input type="button" name="button" id="button" value="选择上传..." class="type-file-button" />
            </span></div>
          <span class="err"></span>
          <p class="notic"><?php echo '请上传背景图片完整URL大小为540px*270px'.$lang['brand_add_support_type'];?>gif,jpg,png</p>
        </dd>
      </dl>

      
            <dl class="row">
        <dt class="tit">品牌小图背景</dt>
        <dd class="opt">
                    <div class="input-file-show"><span class="show"><a class="nyroModal" rel="gal" href="<?php echo $output['brand_array']['brand_xbgpic']; ?>"> <i class="fa fa-picture-o" onMouseOver="toolTip('<img src=<?php echo $output['brand_array']['brand_xbgpic']; ?>>')" onMouseOut="toolTip()"></i></a></span><span class="type-file-box">
            <input class="type-file-file" id="_xbgpic" name="_pic" type="file" size="30" hidefocus="true" title="点击前方预览图可查看大图，点击按钮选择文件并提交表单后上传生效">
            <input type="text" name="brand_xbgpic" id="brand_xbgpic" class="type-file-text" />
            <input type="button" name="button" id="button" value="选择上传..." class="type-file-button" />
            </span></div>
          <span class="err"></span>
          <p class="notic"><?php echo '请上传小背景图片为260px*260px'.$lang['brand_add_support_type'];?>gif,jpg,png</p>
        </dd>
      </dl>
      <dl class="row">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <div class="onoff">
            <label for="brand_recommend1" class="cb-enable <?php if($output['brand_array']['brand_recommend'] == '1'){ ?>selected<?php } ?>" title="<?php echo $lang['feiwa_yes'];?>"><?php echo $lang['feiwa_yes'];?></label>
            <label for="brand_recommend0" class="cb-disable <?php if($output['brand_array']['brand_recommend'] == '0'){ ?>selected<?php } ?>" title="<?php echo $lang['feiwa_no'];?>"><?php echo $lang['feiwa_no'];?></label>
      ...
      <dl class="row">
        <dt class="tit"><?php echo $lang['feiwa_sort'];?></dt>
        <dd class="opt">
...
// 点击查看图片
  ...
  function ajaxFileUpload(id)
  {
    ...
    (
      {
        url : '<?php echo ADMIN_SITE_URL?>/index.php?app=common&feiwa=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_BRAND;?>',
        secureuri:false,
        fileElementId:id,
        dataType: 'json',
        success: function (data, status)
        {
          if (data.status == 1){
          	if(id=='_pic'){
            ajax_form('cutpic','<?php echo $lang['feiwa_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?app=common&feiwa=pic_cut&type=brand&x=150&y=50&resize=1&ratio=3&url='+data.url,690);
            }else if(id=='_bgpic'){
            	 $('#brand_bgpic').val(data.url);
            }else if(id=='_xbgpic'){
            	$('#brand_xbgpic').val(data.url);
            }
          }else{
            alert(data.msg);
          }
      ...
      var error_td = element.parent('dd').children('span.err');
        ...
        rules : {
            brand_name : {
                required : true,
                remote   : {
                    url :'index.php?app=brand&feiwa=ajax&branch=check_brand_name',
                    type:'get',
                    data:{
                        brand_name : function(){
