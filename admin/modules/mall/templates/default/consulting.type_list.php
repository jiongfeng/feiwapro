  ...
  <table class="flex-table">
    ...
    <?php if(!empty($output['type_list'])){ ?>
      ...
      <tr>
        ...
        <td class="handle"><a class="btn red" href="<?php echo urlAdminMall('consulting', 'type_del', array('ct_id' => $value['ct_id']));?>" onclick="if(confirm('删除后将不能恢复，确认删除这  1 项吗？')){return true;} else {return false;}"><i class="fa fa-trash-o"></i>删除</a> <a class="btn blue" href="<?php echo urlAdminMall('consulting', 'type_edit', array('ct_id' => $value['ct_id']));?>"><i class="fa fa-pencil-square-o"></i><?php echo $lang['feiwa_edit'];?></a></td>
...
<script>
    ...
    function fg_operation(name, grid) {
        if (name == 'add') {
            window.location.href = 'index.php?app=consulting&feiwa=type_add';
        }
    }
</script> 
