...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?app=goods&feiwa=get_xml&type=<?php echo $output['type'];?>',
        colModel : [
            ...
            {display: '店铺类型', name : 'is_own_mall', width : 80, sortable : true, align: 'center'},
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=goods&feiwa=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?app=goods&feiwa=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&feiwa=export_csv&type=<?php echo $output['type'];?>&id=' + id;
}


//商品下架
function fg_lonkup(ids) {
    _uri = "index.php?app=goods&feiwa=goods_lockup&id=" + ids;
    CUR_DIALOG = ajax_form('goods_lockup', '违规下架理由', _uri, 640);
}

function fg_sku(commonid) {
    CUR_DIALOG = ajax_form('login','商品"' + commonid +'"的SKU列表','<?php echo urlAdminMall('goods', 'get_goods_sku_list');?>&commonid=' + commonid, 480);
}
// 删除
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?app=goods&feiwa=goods_del', {id:id}, function(data){
            if (data.state) {
...
function fg_verify(ids) {
    _uri = "index.php?app=goods&feiwa=goods_verify&id=" + ids;
