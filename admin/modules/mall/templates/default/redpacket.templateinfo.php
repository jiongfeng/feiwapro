...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?app=redpacket&feiwa=rptlist" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3>平台红包 - 红包详情</h3>
        <h5>平台红包新增与管理</h5>
      </div>
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
            <?php if($output['t_info']['rpacket_t_recommend'] == '1'){ echo $lang['feiwa_yes']; }else{ echo $lang['feiwa_no']; } ?>
        </dd>
      </dl>
      ...
      <dl class="row">
        ...
        <dd class="opt">
            <input type="text" onclick="oCopy(this)" value="<?php echo MALL_SITE_URL.DS;?>index.php?app=redpacket&feiwa=getredpacket&tid=<?php echo $output['t_info']['rpacket_t_id'];?>" readonly class='w350'/>
<script>
	function oCopy(obj){
		obj.select();
...
<script>
$(function(){
    var flexUrl = 'index.php?app=redpacket&feiwa=rplist_xml&tid=<?php echo $output['t_info']['rpacket_t_id'];?>&gtype=<?php echo $output['t_info']['rpacket_t_gettype_key'];?>';
    ...
    $("#build_rp").click(function(){
    	ajaxget("<?php echo urlAdminMall('redpacket','rpbulidpwd',array('tid'=>$output['t_info']['rpacket_t_id']));?>");
    });
});
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = 'index.php?app=redpacket&feiwa=export_step1&tid=<?php echo $output['t_info']['rpacket_t_id'];?>&rid=' + id;
}
</script>